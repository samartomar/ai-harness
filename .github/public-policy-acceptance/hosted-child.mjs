...
for(const[key,value]of [['user.name','github-actions[bot]'],['user.email','41898282+github-actions[bot]@users.noreply.github.com']])assert.equal(spawnSync(git,['config','--local',key,value],{cwd:process.env.GITHUB_WORKSPACE,encoding:'utf8',windowsHide:true}).status,0);
writeFileSync(join(hostedPaths().raw,'operator.json'),JSON.stringify({gitIdentity:'github-actions[bot]',dispatchActor:process.env.GITHUB_ACTOR,run:process.env.GITHUB_RUN_ID,source:process.env.GITHUB_SHA}));
