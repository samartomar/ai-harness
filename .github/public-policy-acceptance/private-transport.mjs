...
import {mkdirSync,writeFileSync,readdirSync,lstatSync} from 'node:fs';
...
export function archive(root){
 ...
 function walk(dir,prefix=''){
  const before=lstatSync(dir,{bigint:true});assert(before.isDirectory()&&!before.isSymbolicLink(),'archive directory link refused');
  for(const entry of readdirSync(dir,{withFileTypes:true}).sort((a,b)=>a.name.localeCompare(b.name,'en'))){
   const path=join(dir,entry.name),rel=safeArchivePath(prefix+entry.name);assert(!entry.isSymbolicLink(),'archive symlink refused');
   if(entry.isDirectory())walk(path,rel+'/');else{assert(entry.isFile()&&files.length<20000,'raw evidence archive limit');const bytes=regularBytes(path,Math.min(128*1024*1024,512*1024*1024-total));total+=bytes.length;files.push({path:rel,sha256:hash(bytes),data:bytes.toString('base64')});}
  }
  const after=lstatSync(dir,{bigint:true});assert(after.isDirectory()&&!after.isSymbolicLink());for(const field of ['ino','dev','mtimeNs','ctimeNs'])assert.equal(after[field],before[field],'archive directory changed');
