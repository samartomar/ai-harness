...
- **Don't assume the host.** Shell, path separator, line endings, and tool-launch
  mechanics all differ by OS. A green run on one OS does not clear another —
  verify platform-specific behavior on the actual target.
...
- `npm`/`npx`/`yarn`/`pnpm` are `.cmd` shims a direct `execFile` can't find; route
  ...
  `is not recognized`.
- Call `setx.exe` directly, never `cmd /c setx` — cmd re-parses `&`/`%`/`^` in the
  value. Create a directory link as a junction via `cmd /c mklink /J` (mklink is a
  cmd builtin with no `.exe`, so it can only run through cmd).
