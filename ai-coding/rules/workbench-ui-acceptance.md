...
| 7 | Kind ledger, rail collapse, inspector tabs | `ui/shell/admin-shell.ts`, pure `ui/kind-ledger.ts` | `new-shell-frame.test.ts`, `kind-ledger.test.ts` | ported: `workbench-ui/tests/sources-screen.test.tsx` |
| 8 | Provenance strip | `ui/shell/new-workbench.ts` `mountProvenance` | `design-foundation.test.ts` | ported: `workbench-ui/tests/org-screen.test.tsx` |
| 9 | Readiness line, adoption recipe, evidence and version drawers | `ui/shell/org-screen.ts` | `new-shell-org.test.ts`, `browser/compact-setup.spec.ts` | ported: `workbench-ui/tests/org-screen.test.tsx` |
...
| 11 | Catalog search, type tabs, source select | `ui/catalog-inventory.ts`, pure `ui/catalog-presentation.ts` | `catalog-browse.test.ts`, `new-shell-sources.test.ts` | ported: `workbench-ui/tests/sources-screen.test.tsx` |
| 12 | Item inspector: details, security scan, policy JSON | `ui/catalog-inventory.ts`, `admin-shell.ts` | `new-shell-inspector.test.ts`, `evidence-display.test.ts` | ported: `workbench-ui/tests/sources-screen.test.tsx` |
| 13 | Developer tool setup | `ui/shell/org-screen.ts`, `ui/developer-tool-selection.ts` | `developer-tool-catalog.test.ts`, `browser/developer-tools.spec.ts` | ported: `workbench-ui/tests/org-screen.test.tsx` |
| 14 | ECC hook controls | `ui/shell/org-screen.ts`, pure `src/org-policy/ecc-hook-controls.ts` | `new-shell-org.test.ts` | ported: `workbench-ui/tests/org-screen.test.tsx` |
| 15 | ECC MCP approval | `ui/shell/acme-screen.ts`, `src/org-policy/ecc-mcp-approval.ts` | `new-shell-acme.test.ts` | ported: `workbench-ui/tests/additions.test.tsx` |
| 16 | Framework curation: add, edit, remove | `ui/shell/acme-screen.ts` | `new-shell-acme.test.ts`, `shell-extracted-semantics.test.ts` | ported: `workbench-ui/tests/additions.test.tsx` |
| 17 | Custom MCP and remote MCP forms | `ui/shell/acme-screen.ts`, `src/org-policy/studio-custom-mcp.ts` | `shell-extracted-semantics.test.ts` | ported: `workbench-ui/tests/additions.test.tsx` |
| 18 | GitHub skill intake, needs the local server | `ui/artifact-intake-runtime.js`, `src/org-policy/ui-server.ts` | `browser/artifact.spec.ts` | ported: `workbench-ui/tests/intake-authority.test.tsx` |
| 19 | Draft review, policy exposure, review badge | `ui/catalog-inventory.ts`, pure `ui/selection-comparison.ts` | `new-shell-changes.test.ts`, `browser/generic-journeys.spec.ts` | ported: `workbench-ui/tests/drafts.test.tsx` |
| 20 | Templates, exclusions, repairs, comparison confirm | `ui/catalog-inventory.ts`, pure `src/org-policy/workbench/selection-engine.ts` | `browser/generic-journeys.spec.ts`, `policy-import.test.ts` | ported: `workbench-ui/tests/drafts.test.tsx` |
| 21 | Import migration message and preview for version 2 policies | `policy-session.ts` `importPolicy`, `ui/schema3-reprojection.ts` | `legacy-download-characterization.test.ts` and the `import-migration.*` goldens | ported: `workbench-ui/tests/drafts.test.tsx` |
| 22 | Artifact intake workspace | `ui/shell/acme-screen.ts`, `ui/artifact-intake-runtime.js`, pure `ui/shell/artifact-intake-model.ts` | `new-shell-download-compat.test.ts`, `browser/new-shell.spec.ts` | ported: `workbench-ui/tests/intake-authority.test.tsx` |
| 23 | Protected bundle authoring: decisions, revocations, digests | `ui/shell/acme-screen.ts`, `src/org-policy/studio-protected-authority.ts` and its runtime | `new-shell-download-compat.test.ts`, `browser/journeys.spec.ts` | ported: `workbench-ui/tests/intake-authority.test.tsx` |
