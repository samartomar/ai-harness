...
- Optional detectors such as SkillSpector, Cisco skill-scanner, Semgrep, Snyk
  Agent Scan, and mcp-scanner are advisory analyzers. Missing analyzers produce
  explicit skips unless policy requires them. AgentShield is not a governed
  detector choice while its package-advertised source repository is unavailable
...
| A newer workflow revision changes an older candidate's controls | Publication and promotion authorization dispatch from the qualified tag revision recorded in the receipt. Installed acceptance dispatches from protected `main`, re-fetches the named tag, and binds its direct tag-ref object, peeled revision, and frozen qualification run/attempt before sealing a separate verifier identity. |
