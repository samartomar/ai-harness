...
the ids new at the pinned SkillSpector and Cisco skill-scanner releases. A
finding under a listed id that every existing route leaves in the detector's
generic bucket is `trust.unreviewed-analyzer-rule`, a `WARN` labelled "new
analyzer rule, not yet reviewed" that never blocks, until the rule is reviewed and
mapped (or confirmed generic) and removed from the list. The label never replaces
a classification Core already makes: a listed id whose message or evidence reaches
egress, legal text or the autonomy review keeps that outcome. The list is
