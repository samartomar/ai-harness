...
> - Local (first-party) sources are graded on aih-native coverage: an *unavailable* deep
>   detector no longer forces UNKNOWN for a local path (it still does for remote sources).
>   Native RED and shape/license rules are unchanged. See
