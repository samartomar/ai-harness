...
_Generated 2026-06-29 on `feat/repo-contract-v1` (v1 PR-1A SHIPPED @ `ee43584`; `main` @ `d675500`). Self-contained: a cold session can execute this without prior chat context. Claims were grounded against live source — file:line references are to this branch. v2 is the **enterprise control plane** that governs the v1 compact contract; v1 (`readProjectContract`/`project.json`/`src/contract/*`, shipped in PR-1A) is the deterministic substrate it reads. **v2 builds ON v1 PR-1A** — `[needs v1]` controls (those reading `commands`/`sensitivePaths`/`contractRef`) require `feat/repo-contract-v1` as their base (or v1 merged to main); `[cold-start]` PRs need nothing._
...
**v1 DEPENDENCY (flagged per-PR):** the `[needs v1]` PRs read `commands`/`sensitivePaths`/`contractRef`, which arrive with v1 PR-1A. **STATUS: PR-1A is SHIPPED** (`ee43584` on `feat/repo-contract-v1`) — `readProjectContract`/`project.json`/`src/contract/*` exist there. So branch v2 off `feat/repo-contract-v1` (or off `main` once v1 merges) and the `[needs v1]` PRs are unblocked; **[cold-start]** PRs build off any base now.
...
| **5** [needs v1] | Posture-grade secrets/path-portability probes via `gradeVerdict` | `src/secrets/index.ts`, `src/contract/index.ts` (`portablePathsCheck`), reuse `gradeVerdict` | vibe=warn, team=fail(secrets,path), enterprise=fail(all)+SARIF; per-control matrix test | M | Med | **#1, v1 PR-1A** |
...
| **12** | P4/P6 MERGE — fold every control's posture verdict into ONE report digest (EXTEND `mcpGovernanceDigest` at `local.ts:130`; do NOT build a parallel spine) + contract-truth digest + `report.contract-untrue` | `src/report/{local,contract}.ts`, `src/internals/verify.ts`; reuse `DigestAction`/`PolicyVerdict` | one digest rolls up secrets/command/risk/MCP/CA verdicts under active posture; omit-undefined; v9 additive slice | M | Med | **#1, #2, #3, #5, #6, v1 PR-1A** |
...
### Open questions to resolve before building (carry into the cold session)
...
5. **Cadence:** confirm the full six-pillar program ships as a sustained sequence (pre-v1 wins → P1 → P2 → P3/P5 → P4/P6 merge) vs a thinner enterprise-MVP (P1 dial + P2 schema + P3 written allowlist + drift probe; defer P5 bundles/VDI). Recommended near-term target = the MVP four (#1, #2, #3, #9).

## START HERE (cold session)

1. **v1 status:** PR-1A is SHIPPED on `feat/repo-contract-v1` (`ee43584`) — `src/contract/{schema,synth,index}.ts` + `readProjectContract` exist. Branch v2 work off `feat/repo-contract-v1` (or off `main` once v1 merges) so `[needs v1]` PRs see the contract reader; `[cold-start]` PRs (#0a/#0b/#0c) build off any base. Don't wait — bank the cold-start wins + the P1 spine first.
