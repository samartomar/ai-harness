...
- **v1.3.1 UX polish**: `bootstrap-ai`/`adopt` now report managed bootloader
  ...
  `verify-bundle`/`verify-release` accept `--posture`, strict `--cli` whitespace
  mistakes hint at comma-separated lists, and `prune` warns when shared selection
  flags are ignored because it diffs committed intent only. (#173, #174, #175,
  #176)
