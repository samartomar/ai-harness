...
| `@aihq/scan` at run time | `src/scan-package/load-scan-package.ts:94` (`loadScanPackageExportsV1`); the execution seam is filled from the installed package by `src/scan-package/load-scan-package.ts:123` (`loadScanExecutionAdapterV1`) | an optional peer dependency, range `src/scan-package/load-scan-package.ts:22` (`SCAN_PACKAGE_PEER_RANGE`); the only `import("@aihq/scan")` in Core, never bundled, each caller's exports checked by `typeof` | package-level, not a consumption refusal (`src/scan-package/load-scan-package.ts:26`): `scan-package-unavailable` when the package cannot be imported, `scan-package-incompatible` when it lacks an export Core calls; both name the install command. Baseline request authoring and publication consumption throw it as `AIH_SCAN_PACKAGE`; the trust scan records it |
| Detectors delegated to the installed Scan | `src/trust/detectors.ts:2249` (`SCAN_DELEGATED_TRUST_DETECTORS`) | a per-detector set, empty while Scan's same-named capabilities are not equivalents; every trust detector runs in Core and each result names its executor (`core-legacy`, `scan`, `precomputed-sarif`, `none`) | a delegated detector the installed Scan cannot run is unavailable with Scan's own refusal or the package refusal, never run by Core. Scan's `detector.aih-native` (`src/trust/detectors.ts:2257`) is recorded only as an identity observation beside Core's detectors, never as findings |
| `@aihq/catalog` at run time | `src/catalog-package/load-catalog-package.ts:207` (`loadCatalogPackageV1`, plus its data-only companion), range `src/catalog-package/load-catalog-package.ts:31` | an optional peer dependency in the exact compatible range `>=0.3.0 <0.4.0`; the only `import("@aihq/catalog")` in Core, never bundled; function readers are checked by `typeof`, data subpaths are resolved through Catalog's own `exports`, and Core validates every returned document against its own schemas and pins | package-level (`src/catalog-package/load-catalog-package.ts:36`): `catalog-package-unavailable` only when the package is not installed; `catalog-package-incompatible` when its version is unreadable/out of range, it cannot be loaded, lacks a reader, or does not publish a required subpath; both name the install command. Thrown as `AIH_CATALOG_PACKAGE` |
...
| Framework plugins at run time | `src/framework-plugin/load-framework-plugin.ts:351` (`loadFrameworkPluginV1`); the closed set `src/framework-plugin/contract-v1.ts:48` (`FRAMEWORK_PLUGIN_PACKAGE_NAMES`) | `@aihq/framework-ecc` and `@aihq/framework-superpowers`, optional peers imported only by literal specifier from that module and never bundled; each must resolve inside Core's install tree with the module its literal import loads inside its own real package directory, implement contract `src/framework-plugin/contract-v1.ts:40` (`FRAMEWORK_PLUGIN_CONTRACT_VERSION`) under its own package name and version and, when Catalog is installed, equal Catalog's `./catalog-framework-plugins.json` identity record. Core accepts only command results its own host services produced | package-level (`src/framework-plugin/load-framework-plugin.ts:53`): `framework-plugin-unavailable` only when the package is not installed; anything else is `framework-plugin-incompatible`, including an installed Catalog Core cannot read identities from; both name the install command. Thrown as `AIH_FRAMEWORK_PLUGIN`; `aih init` reports its Superpowers phase as refused with the reason (coded skip `framework-plugin.unavailable`, failure `framework-plugin.incompatible`) |
