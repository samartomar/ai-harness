...
| Detectors run by the installed Scan | `src/trust/detectors.ts:1463` (`SCAN_DETECTOR_IDS`), the native findings through `src/trust/detectors.ts:2205` (`resolveScanTrustLintRouteV1`), the binding gate's FAST tier through `src/binding/scan-binding-gate.ts:32` (`BINDING_GATE_DETECTOR_ID`), Cisco source shards through `src/trust/cisco-shard-delegation.ts:157` (`runCiscoSourceShardThroughScanV1`) and the baseline preflight through `src/trust/detector-availability.ts:56` (`probeScanDetectorsV1`) | every detector runs in Scan; Core runs none. Core names the profile (`src/trust/detectors.ts:1480` host uv by default, `src/trust/detectors.ts:1482` in-process trust lint, `src/trust/detectors.ts:1484` local SkillSpector) and reads SARIF plus Scan's facts; classification, grading and the gate decision stay in Core. Each result names its executor (`scan`, `precomputed-sarif`, `none`). Core accepts only the analyzer identities it pins in `src/trust/scan-analyzer-identity.ts:33` (`ACCEPTED_SCAN_ANALYZER_IDENTITIES_V1`), checked against both Scan's declaration and the run's evidence; a baseline vet names its analyzers from that table | a missing or incompatible Scan refuses the native findings, the binding gate, the Cisco shard and the preflight as `AIH_SCAN_PACKAGE`; a third-party detector Scan cannot run is unavailable with Scan's own reason, never run by Core. Scan's `detector.aih-native` (`src/trust/detectors.ts:1460`) is recorded only as an identity observation, never as findings |
| `@aihq/catalog` at run time | `src/catalog-package/load-catalog-package.ts:354` (`loadCatalogPackageV1`, plus its data-only companion), range `src/catalog-package/load-catalog-package.ts:32` | an optional peer dependency in the exact compatible range `>=0.3.0 <0.4.0`; the only `import("@aihq/catalog")` in Core, never bundled; function readers are checked by `typeof`, data subpaths are resolved through Catalog's own `exports`, and Core validates every returned document against its own schemas and pins | package-level (`src/catalog-package/load-catalog-package.ts:37`): `catalog-package-unavailable` only when the package is not installed; `catalog-package-incompatible` when its version is unreadable/out of range, it cannot be loaded, lacks a reader, or does not publish a required subpath; both name the install command. Thrown as `AIH_CATALOG_PACKAGE` |
...
| Framework plugins at run time | `src/framework-plugin/load-framework-plugin.ts:381` (`loadFrameworkPluginV1`); the closed set `src/framework-plugin/contract-v1.ts:53` (`FRAMEWORK_PLUGIN_PACKAGE_NAMES`) at the closed-set directories `src/framework-plugin/load-framework-plugin.ts:113` (`BUNDLED_FRAMEWORK_PLUGIN_DIRECTORIES`) | `@aihq/framework-ecc` and `@aihq/framework-superpowers` ship inside `@aihq/core` (never published on their own): Core's tarball carries each plugin's built package under `packages/`, imported only from that module and only from Core's own package root, never bundled into Core's chunks; each package's `package.json` must lie inside Core's own package directory with its entry module inside its own real package directory, implement contract `src/framework-plugin/contract-v1.ts:44` (`FRAMEWORK_PLUGIN_CONTRACT_VERSION`) under its own package name and version and, when Catalog is installed, equal Catalog's `./catalog-framework-plugins.json` identity record. Core accepts only command results its own host services produced | package-level (`src/framework-plugin/load-framework-plugin.ts:56`): `framework-plugin-unavailable` only when the bundled package is missing from the install; anything else is `framework-plugin-incompatible`, including an installed Catalog Core cannot read identities from; both name the `@aihq/core` reinstall command. Thrown as `AIH_FRAMEWORK_PLUGIN`; `aih init` reports its Superpowers phase as refused with the reason (coded skip `framework-plugin.unavailable`, failure `framework-plugin.incompatible`) |
| Historical ECC runtime descriptor | `packages/framework-ecc/src/ecc/runtime-descriptor-resolver.ts:454` (`resolveHistoricalEccRuntimeDescriptorV1`), in the order `packages/framework-ecc/src/ecc/runtime-descriptor-resolver.ts:127` (`HISTORICAL_ECC_RUNTIME_DESCRIPTOR_RESOLUTION_ORDER_V1`) | the `ecc-runtime-descriptor/v1` bytes, accepted from the installed Catalog only by the sha256 `@aihq/framework-ecc` pins at `packages/framework-ecc/src/ecc/runtime-descriptor-resolver.ts:146` (`ACCEPTED_CATALOG_ECC_RUNTIME_DESCRIPTORS_V1`); Catalog is the carrier and its signature is not consulted. Order: a matching verified local source-data receipt, then the installed Catalog; Core's embedded Workbench data is not a descriptor fallback. The result records `descriptorSource` and the stages consulted | `packages/framework-ecc/src/ecc/runtime-descriptor-resolver.ts:156`: absent Catalog is `catalog-package-unavailable`; an unusable installed Catalog is `catalog-package-incompatible`, both surfaced as `AIH_CATALOG_PACKAGE` when no local receipt matches. `catalog-index-refused`, `catalog-runtime-descriptors-refused` (including unknown sidecar versions), `catalog-descriptor-absent`, `catalog-descriptor-unverified` and `catalog-descriptor-not-accepted` are `AIH_TRUST` refusals; none fall back to embedded bytes |
