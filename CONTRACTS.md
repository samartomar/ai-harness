...
| Detectors delegated to the installed Scan | `src/trust/detectors.ts:2247` (`SCAN_DELEGATED_TRUST_DETECTORS`) | a per-detector set, empty while Scan's same-named capabilities are not equivalents; every trust detector runs in Core and each result names its executor (`core-legacy`, `scan`, `precomputed-sarif`, `none`) | a delegated detector the installed Scan cannot run is unavailable with Scan's own refusal or the package refusal, never run by Core. Scan's `detector.aih-native` (`src/trust/detectors.ts:2255`) is recorded only as an identity observation beside Core's detectors, never as findings |
| `@aihq/catalog` at run time | `src/catalog-package/load-catalog-package.ts:152` (`loadCatalogPackageV1`) | an optional peer dependency, range `src/catalog-package/load-catalog-package.ts:29` (`CATALOG_PACKAGE_PEER_RANGE`); the only `import("@aihq/catalog")` in Core, never bundled; the readers Core calls are checked by `typeof`, and the public data subpaths it reads (`./catalog-index.json`, `./catalog-runtime-descriptors.json`) are resolved through Catalog's own `exports` | package-level (`src/catalog-package/load-catalog-package.ts:34`): `catalog-package-unavailable` only when the package is not installed; `catalog-package-incompatible` when it is installed but cannot be loaded, lacks a reader or does not publish a subpath; both name the install command. Thrown as `AIH_CATALOG_PACKAGE` |
| Historical ECC runtime descriptor | `src/ecc/runtime-descriptor-resolver.ts:460` (`resolveHistoricalEccRuntimeDescriptorV1`), in the order `src/ecc/runtime-descriptor-resolver.ts:128` (`HISTORICAL_ECC_RUNTIME_DESCRIPTOR_RESOLUTION_ORDER_V1`) | the `ecc-runtime-descriptor/v1` bytes, accepted from the installed Catalog only by the sha256 Core pins at `src/ecc/runtime-descriptor-resolver.ts:148` (`ACCEPTED_CATALOG_ECC_RUNTIME_DESCRIPTORS_V1`); Catalog is the carrier and its signature is not consulted. Order: a local source-data receipt, then the installed Catalog, then Core's embedded copy only when Catalog is not installed; the result records `descriptorSource` and every stage consulted | `src/ecc/runtime-descriptor-resolver.ts:158`: `catalog-index-refused`, `catalog-runtime-descriptors-refused` (carrying Catalog's own reason, such as an unknown sidecar version), `catalog-descriptor-absent`, `catalog-descriptor-unverified`, `catalog-descriptor-not-accepted`, thrown as `AIH_TRUST`; an installed Catalog that refuses is never answered with the embedded copy |
