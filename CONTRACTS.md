...
| Detectors delegated to the installed Scan | `src/trust/detectors.ts:2233` (`SCAN_DELEGATED_TRUST_DETECTORS`) | a per-detector set, empty while Scan's same-named capabilities are not equivalents; every trust detector runs in Core and each result names its executor (`core-legacy`, `scan`, `precomputed-sarif`, `none`) | a delegated detector the installed Scan cannot run is unavailable with Scan's own refusal or the package refusal, never run by Core. Scan's `detector.aih-native` (`src/trust/detectors.ts:2241`) is recorded only as an identity observation beside Core's detectors, never as findings |
