...
// ---------------------------------------------------------------------------
// Packed-consumer proof of the Core/Scan boundary: Core is built with its own
// tsup config and declaration emit, packed, and installed with
// --ignore-scripts into disposable consumers outside the repository, once
// without Scan and once beside the Scan this checkout installs. The tool asserts
// the Core-only load, the explicit scan-package refusals, the TypeScript
// declarations with skipLibCheck false, the installed-Scan delegation, and that
// the packed dist carries no Scan implementation.
// ---------------------------------------------------------------------------
...
describe("packed Core and the optional @aihq/scan peer", () => {
  it("installs, loads, types and delegates across the package boundary", () => {
    const work = realpathSync(mkdtempSync(join(tmpdir(), "aih-packed-scan-boundary-test-")));
    scratch.push(work);
    ...
    const verified = spawnSync(
      process.execPath,
      [
        join(repoRoot, "tools", "verify-packed-scan-boundary.mjs"),
        "--stage-from",
        repoRoot,
        "--scan",
