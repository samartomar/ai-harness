...
function fixture(): {
  ...
  const checkoutRoot = join(installRoot, "token-optimizer", "v5.13.21");
...
describe("Token Optimizer ordinary adopter runtime", () => {
  ...
  it("writes a digest-bound receipt and reconciles repeated setup idempotently", async () => {
    ...
    const deps = {
      acquire: async (request: {
        ...
        expect(request.source.commit).toBe("e3c0fa6223b1a936bfc32485651f2f03add5c52b");
        return {
          checkoutRoot,
  ...
  it("preserves existing custom settings across repeated concrete configuration", async () => {
    ...
    const runner = fakeRunner(() => {
      // The upstream installer appends wrappers it does not recognize on repeat installation.
  ...
  it("parses the exact 159-record release manifest before rejecting a tampered checkout blob", async () => {
    const values = fixture();
