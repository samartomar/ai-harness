...
import { createHostLoadSurfaceContract, hostLoadSurfaces } from "../../src/methodology/contracts/host.js";
...
beforeEach(() => {
  ...
  for (const path of [".agents", ".codex", "agents", "commands", "hooks", "mcp-configs", "rules", "skills", "scripts"]) {
...
function hostContract() {
  return createHostLoadSurfaceContract({
    ...
    surfaces: hostLoadSurfaces.map((surface) => ({
      surface,
      coverage: surface === "cache-and-session-persistence" ? ("partial" as const) : ("complete" as const),
      evidence: ["fixture"],
...
describe("ECC inert qualification", () => {
  it("qualifies an exact local checkout as data and blocks an unsupported fork tuple", async () => {
    ...
    expect(result.topology).toMatchObject({
      ...
      methodologyClosure: [".agents", ".codex", "agents", "commands", "hooks", "mcp-configs", "rules", "skills"],
    ...
    expect(result.topology.binEntrypoints).toEqual({ ecc: "scripts/ecc.js", "ecc-install": "scripts/install-apply.js" });
