...
// W5 (project framework binding) deliberate ratchet adjustment (maintainer-approved):
// the branch-dense closure classifier + visible-typography reclassifier landed the
...
describe("coverage policy", () => {
  ...
  it("matches the required hosted-runner budgets without weakening Linux", () => {
    ...
    // four-core hosted runner, reserve Chromium's two workers and the packed
    // CLI it starts, so the retained project receives a single worker.
    expect(workbenchRetainedWorkersForParallelAcceptance(4)).toBe(1);
