...
async function expectWheelToMoveDocument(
  page: Page,
  target: Locator,
  label: string,
): Promise<void> {
  ...
  expect(remaining, `${label} needs remaining document content to scroll`).toBeGreaterThan(180);

  await page.mouse.wheel(0, 720);

  await expect
    .poll(() => page.evaluate(() => window.scrollY), {
      message: `${label} wheel must move the document`,
    })
...
test("uses the document for desktop and narrow Workbench scrolling", async ({
  page,
  workbench,
}) => {
  ...
  await expectWheelToMoveDocument(page, sourceRail, "source rail");
  await expectWheelToMoveDocument(page, catalog, "catalog");
  await expectWheelToMoveDocument(page, inspector, "inspector");
  ...
  const deepInspect = page.locator("button.workbench-row-title[data-workbench-expand-id]").nth(35);
  ...
  if (restoredRowBox === null) throw new Error("Expected the opened catalog row to remain visible");
  ...
  await expectWheelToMoveDocument(page, catalog, "narrow catalog");
