...
describe("FsTransaction — removals (aih prune)", () => {
  ...
  it("refuses an asserted authority file that exceeds its transaction byte ceiling", () => {
    const authorityBytes = "bounded authority\n";
    const authority = put("ownership.json", authorityBytes);
    const generated = join(dir, "generated.md");
    const transaction = new FsTransaction();
    transaction.stage(generated, "generated bytes\n");
    transaction.stageAssertion(
      authority,
      createHash("sha256").update(authorityBytes, "utf8").digest("hex"),
      "ownership receipt",
      dir,
      authorityBytes.length - 1,
    );

    expect(() => transaction.commit()).toThrow(/ownership receipt changed before commit/);
    expect(existsSync(generated)).toBe(false);
    expect(readFileSync(authority, "utf8")).toBe(authorityBytes);
  });
...
describe("readRegularFile — the fd-guarded read for scan-discovered paths", () => {
  ...
  it("rejects an invalid descriptor byte ceiling", () => {
    const path = join(dir, "invalid-ceiling.txt");
    writeFileSync(path, "content", "utf8");
    const fd = openSync(path, "r");
    try {
      expect(() => readBoundedFileDescriptor(fd, -1)).toThrow(
