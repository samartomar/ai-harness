 ...
 * W4 adapters will drive, with every user surface byte remaining intact.
 */
...
describe("W3 exit criterion — bind/verify/remove round-trip with the lock in the loop", () => {
  it("binds all mechanisms, locks, removes from the re-read lock, and leaves a clean tree", async () => {
    ...
    const engine = new ClaudeManagedWriteEngine(root)
      ...
      .mcpServer("sample-mcp", { command: "sample-mcp-server" })
      .claudeMdBlock("Routing: sample skills live under .claude/skills/sample.")
      .ownedFile(".claude/rules/sample/routing.md", "# sample routing rule\n");
    ...
    expect(settingsAfterBind.enabledPlugins).toEqual({ "sample@aih-test": true });
    expect(settingsAfterBind.skillOverrides).toEqual({ "gs-review": "off" });
    ...
    expect(readJson(root, CLAUDE_MCP_PATH).mcpServers).toEqual({
      "user-mcp": { command: "u" },
      "sample-mcp": { command: "sample-mcp-server" },
    });
    ...
    expect(existsSync(join(root, ".claude", "rules", "sample", "routing.md"))).toBe(true);
    ...
    // The user-owned deny entry remains untouched.
    ...
    expect(existsSync(join(root, ".claude", "rules", "sample"))).toBe(false);
  ...
  it("re-binding over an existing bind stays idempotent and still round-trips clean", async () => {
    ...
    const bind = () => {
      const engine = new ClaudeManagedWriteEngine(root).jsonField(
        CLAUDE_SETTINGS_PATH,
        "/enabledPlugins/sample@aih-test",
        true,
      );
      return engine.build();
    };
