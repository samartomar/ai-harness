...
Release baseline covered by this guide: `@aihq/harness@5.4.0`. The scoped public security doc documents SLSA v1.2 Build L2 for tagged release artifacts; no Build L3 or formal compliance claim is made.
...
```console
npm install -g @aihq/harness@5.4.0
aih verify-release 5.4.0
```
...
`npm install -g @aihq/harness@5.4.0`); `npm update -g` may stay within the current major. Re-run
`aih verify-release 5.4.0` after an upgrade. Use `--force` only
