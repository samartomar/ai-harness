...
Use this guide when the question is "which `aih` command applies to this situation?" The public `docs/commands.md` page and `aih <command> --help` remain the syntax authorities. This guide maps common workflows across the `@aihq/core@0.1.1` command surface; it does not replace the command reference.
...
| Source | Retained reviewed pin | Example commands |
|---|---|---|
...
those candidates into the approval examples without a later complete green
re-vet.
...
| AWS retired core | Do not preserve `awslabs.core-mcp-server` from an older projection. | It is no longer generated because its diagram-server dependency is entirely yanked. Prefer the hosted AWS Knowledge endpoint after egress approval or qualify Agent Toolkit for AWS. |
| AWS Knowledge / awslabs selected servers | `aih mcp approve aws-knowledge-mcp-server ...`; approve `awslabs.aws-documentation-mcp-server` or `awslabs.aws-iac-mcp-server` only after source/package review. | AWS Knowledge HTTP endpoint is `https://knowledge-mcp.global.api.aws`. Legacy [awslabs/mcp](https://github.com/awslabs/mcp) guidance is refreshed to tag `2026.07.20260728181317` at `536db49a5a5883ab26f8210af90dfc714fee89e7`; prefer Agent Toolkit for AWS for new production adoption. |
