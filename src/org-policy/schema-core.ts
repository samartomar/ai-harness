/**
 * The canonical organization-policy grammar, pure.
 *
 * Everything here is the schema itself — the zod definitions, the types, the
 * digests and the parsers — with no host dependency of any kind, so the same
 * definitions that the CLI enforces also bundle for the browser. The Node
 * loader that reads a policy off disk lives in `./schema.js`, which re-exports
 * every name below; importers keep using `./schema.js`.
 *
 * `tests/org-policy/schema-core-boundary.test.ts` pins both halves of that: the
 * browser bundle of this file, and the re-export.
 */
import { z } from "zod";
...
export function hookCommandDigest(command: string): string {
  return `sha256:${sha256HexOfUtf8(command)}`;
}
