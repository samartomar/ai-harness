...
const state={policy:structuredClone(model.initialPolicy),receipt:null,decision:null,compositionClaims:{},frameworkExclusions:{}};
...
const policyProblems=function(){return schemaErrors(model.schema,state.policy,"").concat(policySemantics(state.policy),selectionDependencySemantics(state.policy),selectionAttributionSemantics(state.policy),kiroTransportSemantics(state.policy),policyTextSemantics(state.policy))};
...
const selectCompositionPart=function(g,part){const framework=eccFramework();if(!framework){return {added:0,blocked:"ECC framework"}}let added=0;let blocked=null;part.componentIds.some(function(id){const asset=framework.assets.find(function(item){return item.id===id});if(!asset){blocked=id;return true}const result=selectFrameworkAssetClosure(g,framework,asset);added+=result.added;if(result.blocked){blocked=result.blocked;return true}return false});return {added:added,blocked:blocked}};
const composeEnterpriseProfile=function(){const previous=structuredClone(state.policy);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}g.supportedClis=(model.catalog.hosts||[]).map(function(host){return host.id});const controls=profileAihControls();controls.forEach(function(control){requestControl(g,control,"enterprise profile")});const composed=model.catalog.enterpriseComposition.parts.filter(function(part){return part.selection==="composed"});let blocked=null;composed.some(function(part){const result=selectCompositionPart(g,part);blocked=result.blocked;return Boolean(blocked)});if(blocked){state.policy=previous;activePreset="custom";announce("Enterprise composition blocked because "+blocked+" cannot be selected with its required closure; nothing changed.",true);render();return}state.policy.minimumPosture="enterprise";const core=composed.reduce(function(total,part){return total+partSelectedCount(part)},0);const additive=model.catalog.enterpriseComposition.parts.filter(function(part){return part.selection==="additive"});markFrameworkSelectionRoots(model.catalog.enterpriseComposition.framework,composed.reduce(function(ids,part){return ids.concat(part.componentIds)},[]));commitPolicy(previous,"Enterprise composed: "+controls.length+" AIH control(s) requested from the always-available set; conditional controls such as Playwright remain yours to add. "+core+" ECC Core component(s) selected as requested intent. "+compositionPartIds("additive")+" further component(s) are offered as additive choices ("+additive.map(function(part){return part.label+" "+part.componentIds.length}).join(", ")+") and are yours to add. "+(frameworkAssetCount()-compositionNamedCount())+" further framework-owned component(s) stay selectable in the inventory. ECC installs and runs all of them; AIH records them. No audit evidence was authored. All "+g.supportedClis.length+" registry CLIs are explicitly sanctioned; requested intent is not effective until runtime evaluation in a target repository.")};
...
const inferFrameworkExclusions=function(policy){const result={},governanceValue=policy&&policy.governance,selections=governanceValue&&Array.isArray(governanceValue.externalSelections)?governanceValue.externalSelections:[];selections.forEach(function(group){if(!Array.isArray(group.roots)){return}const framework=model.catalog.frameworks.find(function(candidate){return candidate.id===group.framework});if(!framework){return}const selected=new Set((group.items||[]).map(function(item){return item.id})),excluded=new Set(),expanded=new Set(),pending=group.roots.map(function(id){return {id:id,includeMembers:true}});while(pending.length){const next=pending.shift(),expansionKey=next.id+"|"+(next.includeMembers?"members":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);const asset=framework.assets.find(function(candidate){return candidate.id===next.id});if(!asset){continue}assetMandatoryRequirementIds(asset).forEach(function(required){if(selected.has(required)){pending.push({id:required,includeMembers:false})}});const suggested=(next.includeMembers?[].concat(asset.members||[]):[]).concat(asset.riders||[]);suggested.forEach(function(id){if(!selected.has(id)){excluded.add(id);return}pending.push({id:id,includeMembers:false})})}if(excluded.size){result[group.framework]=Array.from(excluded)}});return result};
...
const toggleFrameworkSelection=function(key,origin){
  ...
  const previous=structuredClone(state.policy),previousExclusions=structuredClone(state.frameworkExclusions),g=ensureGovernance();
  ...
  if(group&&group.items.some(function(item){return item.id===found.asset.id})){
    ...
    commitPolicy(previous,message);return
  }
  ...
  if(selected.blocked){
    ...
    state.policy=previous;state.frameworkExclusions=previousExclusions;activePreset=previousPreset;
  ...
  markFrameworkSelectionRoots(found.framework.id,[found.asset.id]);
  ...
  if(!commitPolicy(previous,"Selected "+found.asset.id+" with "+requirements.size+relation+": requested intent recorded with pinned sources."+compositionBreakNotice(wasEnterpriseComplete))){state.frameworkExclusions=previousExclusions;render()}
};
...
document.addEventListener("click",function(event){const button=event.target.closest&&event.target.closest("[data-framework-select]");if(!button){return}const key=button.getAttribute("data-framework-select");const fromRail=Boolean(button.closest(".rail"));if(fromRail){revealRailSelectionGroup(key)}toggleFrameworkSelection(key,fromRail?"rail":"center")});
...
byId("import-policy").addEventListener("click",function(){byId("policy-file").click()});byId("policy-file").addEventListener("change",function(event){readFile(event.target,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const problems=schemaErrors(model.schema,value,"").concat(policySemantics(value),selectionDependencySemantics(value),selectionAttributionSemantics(value),kiroTransportSemantics(value),policyTextSemantics(value));if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.policy=value;state.compositionClaims={};state.frameworkExclusions=inferFrameworkExclusions(state.policy);activePreset="custom";announce("Policy imported without transformation after schema and policy-grammar validation.");render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}})});
...
document.addEventListener("click",function(event){if(!event.target.closest||!event.target.closest("#clear-policy")){return}
  state.policy=structuredClone(model.initialPolicy);state.editing=null;state.compositionClaims={};state.frameworkExclusions=inferFrameworkExclusions(state.policy);activePreset="custom";
...
state.frameworkExclusions=inferFrameworkExclusions(state.policy);
buildRail();
render();
</script>
<script>
...
const browserArgumentErrors=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const sources=[];const catalog=governanceValue.catalog||{};["reviewed","custom"].forEach(function(collection){(catalog[collection]||[]).forEach(function(candidate,index){sources.push({source:candidate&&candidate.source,path:"policy.governance.catalog."+collection+"["+index+"]"})})});const approvals=governanceValue.authority&&governanceValue.authority.approvals||[];approvals.forEach(function(approval,index){sources.push({source:approval&&approval.source,path:"policy.governance.authority.approvals["+index+"]"})});sources.forEach(function(entry){const source=entry.source;if(source&&(source.type==="package"||source.type==="stdio")&&!safeBrowserHttpsOrigin(source.registry)){errors.push(validationError(entry.path+".source.registry","must be an exact HTTPS origin"))}if(source&&source.type==="command"&&Array.isArray(source.args)){source.args.forEach(function(argument,index){if(!safeBrowserArgument(argument)){errors.push(validationError(entry.path+".source.args["+index+"]","must be a safe relative argument or exact HTTPS registry/index origin"))}})}});return errors};
  ...
  const browserSemanticErrors=function(policy){return policySemantics(policy).filter(function(error){return !/\.source\.args\[\d+\]: must be a safe relative argument/.test(error)}).concat(selectionDependencySemantics(policy),selectionAttributionSemantics(policy),kiroTransportSemantics(policy),browserArgumentErrors(policy),browserRootErrors(policy))};
