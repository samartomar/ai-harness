...
export function policyStudioHtml(model: PolicyStudioModel): string {
  ...
  const aihMcpIdentityCount = new Set([
    ...aihMcpControlIds,
    ...model.catalog.unavailableMcp.map((item) => item.id),
...
<div class="stage">
  ...
  <div class="work">
    ...
    <main class="plane" id="workbench" tabindex="-1">
      ...
      <section class="gcard grp group" id="surface-aih-mcp-servers" data-open="1" data-owner="AIH" data-groupcard><button type="button" class="grphead" data-group aria-expanded="true"><span class="tw" aria-hidden="true">&#9654;</span><h2>AIH MCP servers</h2><span class="own">AIH</span><span class="ct"></span><span class="meter" aria-hidden="true"></span></button><div class="grpbody"><section class="mcp-catalog-subgroup" aria-labelledby="aih-mcp-servers-heading"><h3 id="aih-mcp-servers-heading">Unique MCP identities <span class="subcount">${String(aihMcpIdentityCount)}</span></h3><p class="help">${String(model.catalog.mcp.length)} selectable projector-backed AIH controls; ${String(sharedAihMcpCount)} are also declared by ECC; ${String(aihMcpDeclarations.length - sharedAihMcpCount)} AIH-owned identities are selectable as requested intent behind a named AIH gate.</p><div class="mcp-catalog-rows" id="mcp-rows"></div></section></div><p class="grpnote">Each MCP identity appears once. A shared row keeps the AIH selection control and carries ECC source provenance; a gated row still accepts requested intent and names the AIH gate — no policy projector, or AIH evidence required — that a selection does not satisfy. A requested control still needs target-repository identity, evidence, authority, safety, ownership and a supported projector before it can become effective. ECC-owned declarations remain in the <b>ECC MCP catalog</b>.</p></section>
      ...
      <section class="gcard grp group" id="surface-ecc-hooks" data-open="0" data-owner="ECC" data-static-count="43" data-groupcard><button type="button" class="grphead" data-group aria-expanded="false"><span class="tw" aria-hidden="true">&#9654;</span><h2>ECC-Guardrails &amp; Safety Hooks</h2><span class="own">ECC</span><span class="ct"></span><span class="meter" aria-hidden="true"></span></button><div class="grpbody stack"><div id="ecc-hook-controls"></div></div><p class="grpnote">ECC owns and executes these 43 pinned hooks. AIH records the supported profile and disabled-hook list, then projects that intent through the supported Claude settings environment. This surface does not claim AIH execution or enforcement.</p></section>
      ...
      <section class="gcard grp group" data-open="0" data-owner="ECC" data-groupcard><button type="button" class="grphead" data-group aria-expanded="false"><span class="tw" aria-hidden="true">&#9654;</span><h2>ECC skills</h2><span class="own">ECC</span><span class="ct"></span><span class="meter" aria-hidden="true"></span></button><div class="grpbody" id="ecc-skill-rows"></div><p class="grpnote">This is the complete source-locked <code>skills/&lt;name&gt;/SKILL.md</code> inventory. Every row records reversible requested intent for its exact pinned source. Selection is not evidence, approval, installation, materialization, or support.</p></section>
...
const state={policy:structuredClone(model.initialPolicy),receipt:null,decision:null,compositionClaims:{},frameworkExclusions:{},structuralFrameworkRoots:{}};
...
const assetMandatoryRequirementIds=function(asset){return Array.from(new Set([].concat(asset&&asset.dependencies||[])))};
const assetRequirementIds=function(asset,includeSuggestions){return Array.from(new Set([].concat(assetMandatoryRequirementIds(asset),includeSuggestions&&asset&&asset.members||[],includeSuggestions&&asset&&asset.riders||[])))};
const assetReachabilityIds=function(asset,includeMembers){return Array.from(new Set([].concat(assetMandatoryRequirementIds(asset),includeMembers&&asset&&asset.members||[],asset&&asset.riders||[])))};
const selectionDependencySemantics=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const selections=Array.isArray(governanceValue.externalSelections)?governanceValue.externalSelections:[];const frameworkIds=selections.map(function(group){return group.framework});if(new Set(frameworkIds).size!==frameworkIds.length){errors.push(validationError("policy.governance.externalSelections","framework records must be unique"))}if(new Set(frameworkIds).size>1){errors.push(validationError("policy.governance.externalSelections","only one framework may be selected at a time"))}selections.forEach(function(group,groupIndex){const selected=new Set();(group.items||[]).forEach(function(item,itemIndex){if(selected.has(item.id)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].items["+itemIndex+"]","duplicate external selection "+item.id))}selected.add(item.id)});const roots=new Set();(group.roots||[]).forEach(function(root,rootIndex){if(roots.has(root)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].roots["+rootIndex+"]","duplicate external selection root "+root))}if(!selected.has(root)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].roots["+rootIndex+"]","selection root "+root+" is not present in items"))}roots.add(root)});const unattributed=new Set();(group.unattributedItems||[]).forEach(function(item,itemIndex){if(unattributed.has(item)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].unattributedItems["+itemIndex+"]","duplicate unattributed external selection "+item))}if(!selected.has(item)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].unattributedItems["+itemIndex+"]","unattributed selection "+item+" is not present in items"))}if(roots.has(item)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].unattributedItems["+itemIndex+"]","unattributed selection "+item+" is also an explicit root"))}unattributed.add(item)});const curated=new Set((governanceValue.externalCuration||[]).filter(function(curation){return curation.framework===group.framework}).flatMap(function(curation){return (curation.items||[]).map(function(item){return item.id})}));const framework=model.catalog.frameworks.find(function(candidate){return candidate.id===group.framework});(group.items||[]).forEach(function(item,itemIndex){const itemPath="policy.governance.externalSelections["+groupIndex+"].items["+itemIndex+"]";if(curated.has(item.id)){errors.push(validationError(itemPath,item.id+" is recorded as both a selection and a curation item"))}const asset=framework&&framework.assets.find(function(candidate){return candidate.id===item.id});if(!asset){errors.push(validationError(itemPath,item.id+" is not present in the pinned "+group.framework+" Workbench catalog"));return}if(item.kind!==asset.kind){errors.push(validationError(itemPath,item.id+" declares kind "+item.kind+", but its pinned kind is "+asset.kind))}const source=item.source;if(source&&typeof source==="object"){if(typeof source.repository==="string"&&source.repository.toLowerCase()!==asset.source.repository.toLowerCase()){errors.push(validationError(itemPath,item.id+" source repository does not match its pinned source repository"))}if(source.commit!==undefined&&source.commit!==asset.source.commit){errors.push(validationError(itemPath,item.id+" source commit does not match its pinned source commit"))}if(source.path!==undefined&&asset.sourcePaths.indexOf(source.path)===-1){errors.push(validationError(itemPath,item.id+" source path does not match its pinned source paths"))}}const includeSuggestions=!Array.isArray(group.roots)||unattributed.has(item.id);assetRequirementIds(asset,includeSuggestions).forEach(function(required){if(!selected.has(required)){errors.push(validationError(itemPath,item.id+" requires "+required+" in the same dependency-closed selection"))}})})});return errors};
const selectionAttributionSemantics=function(policy){const errors=[],governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const selections=Array.isArray(governanceValue.externalSelections)?governanceValue.externalSelections:[];selections.forEach(function(group,groupIndex){if(!Array.isArray(group.roots)){return}const framework=model.catalog.frameworks.find(function(candidate){return candidate.id===group.framework});if(!framework){return}const reachable=new Set(),expanded=new Set(),pending=[].concat(group.roots||[],group.unattributedItems||[]).map(function(id){return {id:id,includeMembers:true}});while(pending.length){const next=pending.shift(),expansionKey=next.id+"|"+(next.includeMembers?"members":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);reachable.add(next.id);const asset=framework.assets.find(function(candidate){return candidate.id===next.id});if(!asset){continue}assetReachabilityIds(asset,next.includeMembers).forEach(function(required){reachable.add(required);pending.push({id:required,includeMembers:false})})}(group.items||[]).forEach(function(item,itemIndex){if(!reachable.has(item.id)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].items["+itemIndex+"]",item.id+" is not reachable from an explicit root or preserved legacy item"))}})});return errors};
const policySemantics=function(policy){const errors=[];const governanceValue=policy&&policy.governance;const registryIds=(model.catalog.hosts||[]).map(function(host){return host.id});if(policy&&policy.minimumPosture==="enterprise"&&(!governanceValue||typeof governanceValue!=="object"||!Array.isArray(governanceValue.supportedClis)||governanceValue.supportedClis.length===0)){errors.push(validationError("policy.governance.supportedClis","enterprise posture requires a non-empty explicit allow-list; current registry ids: "+registryIds.join(", ")+". Paste every id to sanction all supported CLIs; wildcard sentinels are not supported"))}if(!governanceValue||typeof governanceValue!=="object"){return errors}const hasSupportedClis=Array.isArray(governanceValue.supportedClis);const supportedClis=hasSupportedClis?governanceValue.supportedClis:[];if(new Set(supportedClis).size!==supportedClis.length){errors.push(validationError("policy.governance.supportedClis","supported CLI entries must be unique"))}const catalog=governanceValue.catalog||{};const reviewed=Array.isArray(catalog.reviewed)?catalog.reviewed:[];const custom=Array.isArray(catalog.custom)?catalog.custom:[];reviewed.forEach(function(item,index){const path="policy.governance.catalog.reviewed["+index+"]";candidateSemantics(item,path,"reviewed",errors);if(item.source&&item.source.type==="hook"&&(!Array.isArray(item.targets)||item.targets.length!==2||!item.targets.includes("claude")||!item.targets.includes("codex"))){errors.push(validationError(path+".targets","reviewed control targets must exactly match AIH's shipped projector targets: claude, codex"))}});custom.forEach(function(item,index){candidateSemantics(item,"policy.governance.catalog.custom["+index+"]","custom",errors)});const candidates=reviewed.concat(custom);const ids=candidates.map(function(item){return item.id});if(new Set(ids).size!==ids.length){errors.push(validationError("policy.governance.catalog","candidate identifiers must be unique"))}const activations=Array.isArray(governanceValue.activations)?governanceValue.activations:[];const activeIds=activations.map(function(item){return item.candidate});if(new Set(activeIds).size!==activeIds.length){errors.push(validationError("policy.governance.activations","candidate decisions must be unique"))}activations.forEach(function(activation,index){const candidate=candidates.find(function(item){return item.id===activation.candidate});if(!candidate){errors.push(validationError("policy.governance.activations["+index+"]","references an unknown candidate"))}else if(Array.isArray(activation.targets)&&activation.targets.some(function(target){return !candidate.targets.includes(target)})){errors.push(validationError("policy.governance.activations["+index+"]","targets exceed candidate support"))}const reviewedCandidate=reviewed.find(function(item){return item.id===activation.candidate});if(reviewedCandidate&&hasSupportedClis){const sanctionedTargets=reviewedCandidate.targets.filter(function(target){return supportedClis.includes(target)});if(sanctionedTargets.length===0){errors.push(validationError("policy.governance.activations["+index+"].targets",activation.candidate+" has no projector for the organization-sanctioned CLI set "+supportedClis.join(", ")+"; control projector targets: "+reviewedCandidate.targets.join(", ")))}else if(!Array.isArray(activation.targets)||activation.targets.length!==sanctionedTargets.length||sanctionedTargets.some(function(target){return !activation.targets.includes(target)})){errors.push(validationError("policy.governance.activations["+index+"].targets","activation targets for "+activation.candidate+" must exactly match the organization-sanctioned projector targets: "+sanctionedTargets.join(", ")))}}});const frameworks=activations.filter(function(activation){return activation.state==="active"&&candidates.some(function(candidate){return candidate.id===activation.candidate&&candidate.kind==="framework"})});if(frameworks.length>1){errors.push(validationError("policy.governance.activations","only one framework intent may be active"))}const approvals=governanceValue.authority&&Array.isArray(governanceValue.authority.approvals)?governanceValue.authority.approvals:[];if(new Set(approvals.map(function(item){return item.id})).size!==approvals.length){errors.push(validationError("policy.governance.authority.approvals","approval identifiers must be unique"))}approvals.forEach(function(approval,index){sourceSemantics(approval.source,"policy.governance.authority.approvals["+index+"].source",errors);isoTime(approval.notBefore,"policy.governance.authority.approvals["+index+"].notBefore",errors);isoTime(approval.expiresAt,"policy.governance.authority.approvals["+index+"].expiresAt",errors)});const curation=Array.isArray(governanceValue.externalCuration)?governanceValue.externalCuration:[];if(new Set(curation.map(function(item){return item.framework})).size!==curation.length){errors.push(validationError("policy.governance.externalCuration","framework records must be unique"))}curation.forEach(function(group,groupIndex){const itemKeys=(group.items||[]).map(function(item){safePolicyText(item.id,"policy.governance.externalCuration["+groupIndex+"].items id",errors);safePath(item.source&&item.source.path,"policy.governance.externalCuration["+groupIndex+"].items path",errors);safePolicyText(item.audit&&item.audit.record,"policy.governance.externalCuration["+groupIndex+"].items audit record",errors);if(item.clarification!==undefined){safePolicyText(item.clarification,"policy.governance.externalCuration["+groupIndex+"].items clarification",errors)}return item.kind+"\\u0000"+item.id});if(new Set(itemKeys).size!==itemKeys.length){errors.push(validationError("policy.governance.externalCuration["+groupIndex+"].items","kind/id pairs must be unique"))}});(policy.trust&&Array.isArray(policy.trust.baselineOverrides)?policy.trust.baselineOverrides:[]).forEach(function(item,index){safePath(item.bundle,"policy.trust.baselineOverrides["+index+"].bundle",errors);isoTime(item.approvedAt,"policy.trust.baselineOverrides["+index+"].approvedAt",errors)});return errors};
...
const policyProblems=function(){return schemaErrors(model.schema,state.policy,"").concat(policySemantics(state.policy),managedMcpPolicySemantics(state.policy),aihMcpRequestSemantics(state.policy),selectionDependencySemantics(state.policy),selectionAttributionSemantics(state.policy),kiroTransportSemantics(state.policy),policyTextSemantics(state.policy))};
let policyValidator=policyProblems;
const canonicalImportValue=function(value){if(Array.isArray(value)){return value.map(canonicalImportValue)}if(value&&typeof value==="object"){return Object.keys(value).sort().reduce(function(record,key){record[key]=canonicalImportValue(value[key]);return record},{})}return value};
const exactImportValue=function(left,right){return JSON.stringify(canonicalImportValue(left))===JSON.stringify(canonicalImportValue(right))};
const managedMcpControlById=function(){return new Map(model.catalog.mcp.map(function(item){return [item.id,item.control]}))};
const expectedManagedMcpCandidate=function(control){return {id:control.id,kind:control.kind,description:"AIH-provided governed control",capabilities:[],risks:[],source:control.source,targets:control.targets,projector:control.projector,lifecycle:control.lifecycle,evidence:{record:"aih-"+control.id}}};
const managedMcpServers=function(policy){const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return []}const reviewed=governanceValue.catalog&&Array.isArray(governanceValue.catalog.reviewed)?governanceValue.catalog.reviewed:[];const active=new Set((Array.isArray(governanceValue.activations)?governanceValue.activations:[]).filter(function(item){return item&&item.state==="active"}).map(function(item){return item.candidate}));return Array.from(new Set(reviewed.filter(function(candidate){return active.has(candidate.id)&&candidate.kind==="mcp"&&candidate.source&&candidate.source.type==="mcp"}).map(function(candidate){return candidate.source.server}))).sort()};
const managedMcpPolicySemantics=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const reviewed=governanceValue.catalog&&Array.isArray(governanceValue.catalog.reviewed)?governanceValue.catalog.reviewed:[];const active=new Set((Array.isArray(governanceValue.activations)?governanceValue.activations:[]).filter(function(item){return item&&item.state==="active"}).map(function(item){return item.candidate}));const currentById=managedMcpControlById();reviewed.forEach(function(candidate,index){if(!candidate||!active.has(candidate.id)||candidate.kind!=="mcp"||!candidate.source||candidate.source.type!=="mcp"){return}const path="policy.governance.catalog.reviewed["+index+"]";const control=currentById.get(candidate.id);if(!control){errors.push(validationError(path,candidate.id+" is not present in the current projectable managed MCP catalog"));return}if(!exactImportValue(candidate,expectedManagedMcpCandidate(control))){errors.push(validationError(path,candidate.id+" does not exactly match the current managed MCP catalog record"))}});const selected=managedMcpServers(policy);const mcp=policy&&policy.mcp;const allowed=mcp&&Array.isArray(mcp.allowedServers)?Array.from(new Set(mcp.allowedServers)).sort():[];if(selected.length===0){if(mcp&&(mcp.allowManagedOnly===true||allowed.length)){errors.push(validationError("policy.mcp","center-panel MCP authority is empty, so managed MCP projection must be disabled and its allow-list empty"))}return errors}if(!mcp||mcp.allowManagedOnly!==true){errors.push(validationError("policy.mcp.allowManagedOnly","selected center-panel MCP controls require managed MCP projection"))}if(JSON.stringify(allowed)!==JSON.stringify(selected)){errors.push(validationError("policy.mcp.allowedServers","must exactly match selected center-panel MCP controls: "+selected.join(", ")))}return errors};
/* The browser mirror of the policy grammar's request rules, plus the one rule
   only a build can state: an identity this Core build ships as a selectable
   control is selected as a control, never recorded as a request. */
const aihMcpRequestSemantics=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const requests=Array.isArray(governanceValue.aihMcpRequests)?governanceValue.aihMcpRequests:[];const path="policy.governance.aihMcpRequests";const ids=requests.map(function(item){return item&&item.id});const duplicate=ids.find(function(id,index){return ids.indexOf(id)!==index});const order=model.catalog.aihMcpRequestIds;if(duplicate!==undefined){errors.push(validationError(path,"AIH MCP request "+duplicate+" is duplicated"))}else if(ids.some(function(id,index){return index>0&&order.indexOf(id)<=order.indexOf(ids[index-1])})){errors.push(validationError(path,"AIH MCP requests must follow the pinned AIH-owned MCP declaration order"))}const catalog=governanceValue.catalog||{};const candidates=new Set([].concat(Array.isArray(catalog.reviewed)?catalog.reviewed:[],Array.isArray(catalog.custom)?catalog.custom:[]).map(function(item){return item&&item.id}));requests.forEach(function(item,index){const id=item&&item.id;const itemPath=path+"["+index+"]";if(candidates.has(id)){errors.push(validationError(itemPath,id+" is recorded as both an AIH MCP request and a policy candidate; one identity keeps one record — remove either the request or the candidate"))}if(model.catalog.mcp.some(function(control){return control.id===id})){errors.push(validationError(itemPath,id+" is recorded as an AIH MCP request, but this Core build ships a selectable AIH control for it; select the control instead"))}});return errors};
/* Core 0.4.3 could emit an Enterprise Workbench policy before the managed MCP
   gate was synchronized. Only that exact, source-bound profile footprint may
   be repaired: current projector-backed candidates must equal the catalog's
   own generated records, and at least one activation must retain the profile
   provenance. Hosted identities that no longer have a projector are removed,
   never promoted, and the administrator is told to review the migrated bytes. */
const legacyWorkbenchMcpMigration=function(policy){if(!policy||typeof policy!=="object"||Object.prototype.hasOwnProperty.call(policy,"mcp")||policy.minimumPosture!=="enterprise"){return null}const g=policy.governance;if(!g||typeof g!=="object"||!g.catalog||!Array.isArray(g.catalog.reviewed)||!Array.isArray(g.activations)){return null}const active=new Map(g.activations.filter(function(item){return item&&item.state==="active"}).map(function(item){return [item.candidate,item]}));const activeMcp=g.catalog.reviewed.filter(function(candidate){return candidate&&candidate.kind==="mcp"&&candidate.source&&candidate.source.type==="mcp"&&active.has(candidate.id)});if(activeMcp.length===0){return null}const legacyOrigins=/^Requested by: (?:enterprise profile|administrator)(?:, (?:enterprise profile|administrator))*$/;if(!activeMcp.some(function(candidate){const activation=active.get(candidate.id),clarification=String(activation&&activation.clarification||"");return clarification.indexOf("Requested by: ")===0&&clarification.slice("Requested by: ".length).split(", ").includes("enterprise profile")})){return null}if(!activeMcp.every(function(candidate){const activation=active.get(candidate.id);return activation&&legacyOrigins.test(String(activation.clarification||""))})){return null}const currentById=new Map(model.catalog.mcp.map(function(item){return [item.id,item.control]})),unavailableById=new Map(model.catalog.unavailableMcp.map(function(item){return [item.id,item]}));const retained=[],removed=[];for(const candidate of activeMcp){const control=currentById.get(candidate.id);if(!control){removed.push(candidate.id);continue}const expected={id:control.id,kind:control.kind,description:"AIH-provided governed control",capabilities:[],risks:[],source:control.source,targets:control.targets,projector:control.projector,lifecycle:control.lifecycle,evidence:{record:"aih-"+control.id}};if(!exactImportValue(candidate,expected)){return null}const activation=active.get(candidate.id);if(!exactImportValue(activation,{candidate:control.id,state:"active",targets:control.targets,clarification:activation.clarification})){return null}retained.push(control.source.server)}const migrated=structuredClone(policy);if(removed.length){const removedIds=new Set(removed);migrated.governance.catalog.reviewed=migrated.governance.catalog.reviewed.filter(function(candidate){return !removedIds.has(candidate.id)});migrated.governance.activations=migrated.governance.activations.filter(function(activation){return !removedIds.has(activation.candidate)})}if(retained.length){migrated.mcp={allowedServers:Array.from(new Set(retained)).sort(),allowManagedOnly:true}}const unavailableRemoved=removed.filter(function(id){return unavailableById.has(id)}).sort(),absentRemoved=removed.filter(function(id){return !unavailableById.has(id)}).sort();const removalMessage=(absentRemoved.length?"; non-projectable MCP authority removed: "+absentRemoved.join(", "):"")+(unavailableRemoved.length?"; unavailable AIH-owned MCP authority removed: "+unavailableRemoved.join(", ")+" (no current protected Scanner evidence record)":"");const message="Legacy Workbench policy migrated: managed MCP projection restored"+(retained.length?" for "+Array.from(new Set(retained)).sort().join(", "):" with no projectable MCP authority")+removalMessage+". Review and download this migrated policy.";return {policy:migrated,message:message}};
const expectedReviewedCandidate=function(control){return {id:control.id,kind:control.kind,description:"AIH-provided governed control",capabilities:[],risks:[],source:control.source,targets:control.targets,projector:control.projector,lifecycle:control.lifecycle,evidence:{record:"aih-"+control.id}}};
const legacyWorkbenchTargetMigration=function(policy){if(!policy||typeof policy!=="object"){return null}const g=policy.governance;if(!g||typeof g!=="object"||!Array.isArray(g.supportedClis)||!g.supportedClis.length||!g.catalog||!Array.isArray(g.catalog.reviewed)||!Array.isArray(g.activations)){return null}const controls=[].concat(model.catalog.mcp.map(function(item){return item.control}),model.catalog.hooks.map(function(item){return item.control}));const controlById=new Map(controls.map(function(control){return [control.id,control]})),candidateById=new Map(g.catalog.reviewed.map(function(candidate){return [candidate.id,candidate]}));const migrated=structuredClone(policy),changed=[];migrated.governance.activations.forEach(function(activation){const control=controlById.get(activation.candidate),candidate=candidateById.get(activation.candidate);if(!control||!candidate||!exactImportValue(candidate,expectedReviewedCandidate(control))||!exactImportValue(activation.targets,control.targets)){return}const sanctioned=control.targets.filter(function(target){return g.supportedClis.includes(target)});if(sanctioned.length&&sanctioned.length<control.targets.length){activation.targets=sanctioned;changed.push(control.id)}});if(!changed.length){return null}return {policy:migrated,message:"Legacy Workbench policy migrated: activation targets narrowed to the sanctioned projector intersection for "+changed.join(", ")+". Catalog support metadata and imported authority records were preserved; review and download the migrated policy."}};
const preparePolicyImport=function(value,problemsFor){const messages=[];const mcpMigration=legacyWorkbenchMcpMigration(value);let imported=mcpMigration?mcpMigration.policy:value;if(mcpMigration){messages.push(mcpMigration.message)}const targetMigration=legacyWorkbenchTargetMigration(imported);if(targetMigration){imported=targetMigration.policy;messages.push(targetMigration.message)}const problems=problemsFor(imported);if(problems.length){throw new Error(problems.slice(0,3).join("; "))}return {policy:imported,message:messages.length?messages.join(" "):"Policy imported without transformation after schema and policy-grammar validation."}};
const syncManagedMcpPolicy=function(){const selected=managedMcpServers(state.policy);if(selected.length){state.policy.mcp=Object.assign({},state.policy.mcp||{},{allowedServers:selected,allowManagedOnly:true});return}if(!state.policy.mcp){return}const mcp=state.policy.mcp;const keepsOtherPolicy=(Array.isArray(mcp.approvals)&&mcp.approvals.length>0)||(Array.isArray(mcp.incumbentHosts)&&mcp.incumbentHosts.length>0)||typeof mcp.githubHost==="string"||(Array.isArray(mcp.disabledServers)&&mcp.disabledServers.length>0);if(keepsOtherPolicy){state.policy.mcp=Object.assign({},mcp,{allowedServers:[],allowManagedOnly:false})}else{delete state.policy.mcp}};
const commitPolicy=function(previous,message){syncManagedMcpPolicy();const problems=policyValidator();if(problems.length){state.policy=previous;announce("Policy change rejected: "+problems.slice(0,3).join("; "),true);render();return false}announce(message);render();return true};
...
const compactState=function(kind,status){if(kind==="requested"){return "Selected"}if(kind==="blocked"){return "Blocked"}if(kind==="approval"){return "Approval"}if(kind==="pending"){return status.indexOf("Disabled")===0?"Disabled":"Awaiting"}if(kind==="external"){return status.indexOf("Selectable")===0?"Selectable":"External"}return "External"};
...
/* Which gate a requestable AIH-owned identity sits behind is build knowledge
   read from the catalog here, never a note frozen into the exported bytes. */
const aihMcpRequestGateEntry=function(id){return model.catalog.unavailableMcp.find(function(item){return item.id===id})||model.catalog.nonProjectableMcp.find(function(item){return item.id===id})};
const aihMcpGateLabel=function(id){const gate=aihMcpRequestGateEntry(id);return gate&&gate.configuredIdentity?"AIH evidence required":"not policy-projectable"};
const aihMcpRequestRoute=function(id){const gate=aihMcpRequestGateEntry(id);return gate&&gate.configuredIdentity?"Next: this request stays not effective until AIH publishes a current protected Scanner evidence record for "+gate.configuredIdentity+"; aih policy evaluate <root> --no-log --json reports the request meanwhile.":"Next: aih policy evaluate <root> --no-log --json reports the request in the target repository; aih policy project never writes this identity, and under a governed policy aih mcp fails closed."};
const aihMcpRequests=function(){const value=governance().aihMcpRequests;return Array.isArray(value)?value:[]};
const isAihMcpRequested=function(id){return aihMcpRequests().some(function(item){return item.id===id})};
const requestProvenance=function(id){return isAihMcpRequested(id)?PROVENANCE_PREFIX+"administrator":""};
...
const profileAihControls=function(){return [].concat(model.catalog.mcp.filter(function(item){return item.availability==="always"}).map(function(item){return item.control}),model.catalog.hooks.map(function(item){return item.control}))};
const unavailableAihMcpSummary=function(){const ids=model.catalog.unavailableMcp.map(function(item){return item.id.charAt(0).toUpperCase()+item.id.slice(1)});return ids.length?ids.join(", ")+" remain"+(ids.length===1?"s":"")+" unavailable until current protected Scanner evidence exists. ":""};
...
const activationTargetText=function(id){const activation=governance().activations.find(function(item){return item.candidate===id});return activation?" Requested targets: "+activation.targets.join(", ")+".":""};
const hookDisclosure=function(hook){return "Fires on "+hook.behaviour.trigger+"; records "+hook.behaviour.records+" to "+hook.behaviour.artifact+". "+hook.behaviour.failureMode+". Projector "+hook.control.projector+", supported targets "+hook.control.targets.join(" and ")+", pinned script "+hook.control.source.scriptDigest+"."+activationTargetText(hook.id)};
...
const sanctionedControlTargets=function(g,control){if(!Array.isArray(g.supportedClis)){return control.targets.slice()}return control.targets.filter(function(target){return g.supportedClis.includes(target)})};
const unsupportedControlScope=function(g,control){return control.id+" has no projector for the organization-sanctioned CLI set "+g.supportedClis.join(", ")+"; control projector targets: "+control.targets.join(", ")};
/* Returns a precise problem when the organization sanctions none of the
   control's projector targets; otherwise callers remain idempotent. */
const requestControl=function(g,control,origin){const targets=sanctionedControlTargets(g,control);if(!targets.length){return unsupportedControlScope(g,control)}if(g.catalog.reviewed.some(function(item){return item.id===control.id})){const activation=g.activations.find(function(item){return item.candidate===control.id});if(activation){activation.targets=targets;recordOrigin(activation,origin)}return null}g.catalog.reviewed.push(expectedReviewedCandidate(control));g.activations.push({candidate:control.id,state:"active",targets:targets,clarification:PROVENANCE_PREFIX+origin});return null};
const rebindReviewedActivationTargets=function(g){if(!Array.isArray(g.supportedClis)){return null}for(const activation of g.activations){const candidate=g.catalog.reviewed.find(function(item){return item.id===activation.candidate});if(!candidate){continue}const targets=candidate.targets.filter(function(target){return g.supportedClis.includes(target)});if(!targets.length){return unsupportedControlScope(g,candidate)}activation.targets=targets}return null};
const toggleReviewed=function(id){const control=aihControls().find(function(item){return item.id===id});if(!control){return}const previous=structuredClone(state.policy);const g=ensureGovernance();if(activePreset==="vibe"||activePreset==="enterprise"){activePreset="custom"}const existing=g.catalog.reviewed.some(function(item){return item.id===id});if(!existing){const problem=requestControl(g,control,"administrator");if(problem){state.policy=previous;announce("Policy change rejected: "+problem,true);render();return}commitPolicy(previous,"Requested intent added for "+sanctionedControlTargets(g,control).join(", ")+"; it is not active until runtime evaluation.");return}const activation=g.activations.find(function(item){return item.candidate===id});const origins=activationOrigins(activation);const administrator=origins.indexOf("administrator");if(activation&&administrator!==-1&&origins.length>1){origins.splice(administrator,1);activation.clarification=PROVENANCE_PREFIX+origins.join(", ");commitPolicy(previous,"Administrator request removed; "+id+" remains requested by "+origins.join(", ")+".");return}g.catalog.reviewed=g.catalog.reviewed.filter(function(item){return item.id!==id});g.activations=g.activations.filter(function(item){return item.candidate!==id});commitPolicy(previous,"Requested intent removed; the exported policy no longer records "+id+".")};
/* Recording requested intent over a gated AIH-owned identity. The gate is a
   label and a next route, never satisfied by the selection: nothing here
   creates a candidate, an activation, or an allow-list entry. */
const toggleMcpRequest=function(id){const gate=aihMcpRequestGateEntry(id);if(!gate){return}const previous=structuredClone(state.policy);const g=ensureGovernance();if(activePreset==="vibe"||activePreset==="enterprise"){activePreset="custom"}const current=Array.isArray(g.aihMcpRequests)?g.aihMcpRequests:[];if(current.some(function(item){return item.id===id})){const remaining=current.filter(function(item){return item.id!==id});if(remaining.length){g.aihMcpRequests=remaining}else{delete g.aihMcpRequests}commitPolicy(previous,"Requested intent removed; the exported policy no longer records "+id+".");return}const order=model.catalog.aihMcpRequestIds;g.aihMcpRequests=current.concat([{id:id,clarification:PROVENANCE_PREFIX+"administrator"}]).sort(function(left,right){return order.indexOf(left.id)-order.indexOf(right.id)});commitPolicy(previous,"Requested intent recorded for "+id+"; the "+(gate.configuredIdentity?"AIH evidence":"policy projector")+" gate still holds, so it is not effective and grants nothing.")};
/* Vibe selects every individually governable catalog component: every
   always-available AIH control is requested and every framework-owned component is selected as requested
   ...
   author one. */
const composeVibeProfile=function(){const previous=structuredClone(state.policy);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const controls=profileAihControls();let controlProblem=null;controls.some(function(control){controlProblem=requestControl(g,control,"vibe profile");return Boolean(controlProblem)});if(controlProblem){state.policy=previous;activePreset="custom";announce("Vibe composition blocked because "+controlProblem+"; nothing changed.",true);render();return}/* Bounded by the one-framework rule: Vibe composes every individually governable component in the framework
...
const selected=g.externalSelections.reduce(function(total,group){return total+group.items.length},0);markFrameworkSelectionRoots(chosen.id,chosen.assets.map(function(asset){return asset.id}));commitPolicy(previous,"Vibe composed: "+controls.length+" AIH control(s) requested and "+selected+" of "+chosen.assets.length+" "+chosen.id+" component(s) selected as requested intent with their pinned sources - "+chosen.id+" installs and runs them, AIH records them. A policy selects from only one framework at a time, so "+excluded+" component(s) in the other framework stay listed and unselected. No audit evidence was authored; each selection still owes its own. "+g.catalog.custom.length+" custom candidate(s) stay blocked. Requested intent is not effective until runtime evaluation in a target repository.")};
...
const selectCompositionPart=function(g,part,structuralOnly){const framework=eccFramework();if(!framework){return {added:0,blocked:"ECC framework"}}let added=0;let blocked=null;part.componentIds.some(function(id){const asset=framework.assets.find(function(item){return item.id===id});if(!asset){blocked=id;return true}const result=structuralOnly?selectFrameworkAssetStructure(g,framework,asset):selectFrameworkAssetClosure(g,framework,asset);added+=result.added;if(result.blocked){blocked=result.blocked;return true}return false});return {added:added,blocked:blocked}};
const composeEnterpriseProfile=function(){const previous=structuredClone(state.policy),previousStructural=structuredClone(state.structuralFrameworkRoots);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}g.supportedClis=(model.catalog.hosts||[]).map(function(host){return host.id});const controls=profileAihControls();let controlProblem=null;controls.some(function(control){controlProblem=requestControl(g,control,"enterprise profile");return Boolean(controlProblem)});if(controlProblem){state.policy=previous;state.structuralFrameworkRoots=previousStructural;activePreset="custom";announce("Enterprise composition blocked because "+controlProblem+"; nothing changed.",true);render();return}const composed=model.catalog.enterpriseComposition.parts.filter(function(part){return part.selection==="composed"});let blocked=null;composed.some(function(part){const result=selectCompositionPart(g,part,true);blocked=result.blocked;return Boolean(blocked)});if(blocked){state.policy=previous;state.structuralFrameworkRoots=previousStructural;activePreset="custom";announce("Enterprise composition blocked because "+blocked+" cannot be selected with its required closure; nothing changed.",true);render();return}state.policy.minimumPosture="enterprise";const core=composed.reduce(function(total,part){return total+partSelectedCount(part)},0);const additive=model.catalog.enterpriseComposition.parts.filter(function(part){return part.selection==="additive"});const coreRoots=composed.reduce(function(ids,part){return ids.concat(part.componentIds)},[]);markFrameworkSelectionRoots(model.catalog.enterpriseComposition.framework,coreRoots);markStructuralFrameworkRoots(model.catalog.enterpriseComposition.framework,coreRoots);if(!commitPolicy(previous,"Enterprise composed: "+controls.length+" AIH control(s) requested from the always-available set. "+unavailableAihMcpSummary()+core+" structural ECC Core module(s) selected as requested intent; their Agent and Skill members remain unselected until you choose them. "+compositionPartIds("additive")+" further component(s) are offered as additive choices ("+additive.map(function(part){return part.label+" "+part.componentIds.length}).join(", ")+") and are yours to add. "+(frameworkAssetCount()-compositionNamedCount())+" further framework-owned component(s) stay selectable in the inventory. ECC installs and runs selected components; AIH records them. No audit evidence was authored. All "+g.supportedClis.length+" registry CLIs are explicitly sanctioned; requested intent is not effective until runtime evaluation in a target repository.")){state.structuralFrameworkRoots=previousStructural;render()}};
...
const preserveRootlessSelection=function(group){if(Array.isArray(group.roots)){return}if(!Array.isArray(group.unattributedItems)){group.unattributedItems=group.items.map(function(item){return item.id})}group.roots=[]};
/* Center inventory choices are the administrator's final authority for this
   authoring session. A left-rail composition helper may suggest dependencies,
   but it may not silently restore an item the administrator explicitly removed.
   Exclusions are authoring state, not policy claims: imports and Clear reset
   them, and an explicit center re-selection removes the exclusion. */
const frameworkExclusionIds=function(frameworkId){const value=state.frameworkExclusions[frameworkId];return Array.isArray(value)?value:[]};
const frameworkAssetExcluded=function(frameworkId,id){return frameworkExclusionIds(frameworkId).indexOf(id)!==-1};
const excludeFrameworkAsset=function(frameworkId,id){if(frameworkAssetExcluded(frameworkId,id)){return}state.frameworkExclusions[frameworkId]=frameworkExclusionIds(frameworkId).concat([id])};
const restoreFrameworkAsset=function(frameworkId,id){const remaining=frameworkExclusionIds(frameworkId).filter(function(item){return item!==id});if(remaining.length){state.frameworkExclusions[frameworkId]=remaining}else{delete state.frameworkExclusions[frameworkId]}};
const structuralFrameworkRootIds=function(frameworkId){const value=state.structuralFrameworkRoots[frameworkId];return Array.isArray(value)?value:[]};
const frameworkRootIsStructural=function(frameworkId,id){return structuralFrameworkRootIds(frameworkId).indexOf(id)!==-1};
const markStructuralFrameworkRoots=function(frameworkId,ids){const roots=new Set(structuralFrameworkRootIds(frameworkId));ids.forEach(function(id){roots.add(id)});state.structuralFrameworkRoots[frameworkId]=Array.from(roots)};
const restoreSuggestedFrameworkRoot=function(frameworkId,id){const remaining=structuralFrameworkRootIds(frameworkId).filter(function(item){return item!==id});if(remaining.length){state.structuralFrameworkRoots[frameworkId]=remaining}else{delete state.structuralFrameworkRoots[frameworkId]}};
const inferStructuralFrameworkRoots=function(policy){const result={},governanceValue=policy&&policy.governance,selections=governanceValue&&Array.isArray(governanceValue.externalSelections)?governanceValue.externalSelections:[];selections.forEach(function(group){if(!Array.isArray(group.roots)){return}const framework=model.catalog.frameworks.find(function(candidate){return candidate.id===group.framework});if(!framework){return}const selected=new Set((group.items||[]).map(function(item){return item.id}));const structural=group.roots.filter(function(id){const asset=framework.assets.find(function(candidate){return candidate.id===id});if(!asset||asset.kind!=="module"){return false}return [].concat(asset.members||[],asset.riders||[]).every(function(member){return !selected.has(member)})});if(structural.length){result[group.framework]=structural}});return result};
/* An explicit-root policy already carries the administrator's final center
   choices: an omitted optional member or rider is an exclusion, not missing
   structural closure. Reconstruct that authoring state on reopen/import so a
   later rail toggle cannot silently restore the omitted suggestion. */
const inferFrameworkExclusions=function(policy,structuralRoots){const result={},governanceValue=policy&&policy.governance,selections=governanceValue&&Array.isArray(governanceValue.externalSelections)?governanceValue.externalSelections:[];selections.forEach(function(group){if(!Array.isArray(group.roots)){return}const framework=model.catalog.frameworks.find(function(candidate){return candidate.id===group.framework});if(!framework){return}const selected=new Set((group.items||[]).map(function(item){return item.id})),structural=new Set(structuralRoots&&structuralRoots[group.framework]||[]),excluded=new Set(),expanded=new Set(),pending=group.roots.map(function(id){return {id:id,includeMembers:!structural.has(id)}});while(pending.length){const next=pending.shift(),expansionKey=next.id+"|"+(next.includeMembers?"members":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);const asset=framework.assets.find(function(candidate){return candidate.id===next.id});if(!asset){continue}const mandatory=new Set(assetMandatoryRequirementIds(asset));assetReachabilityIds(asset,next.includeMembers).forEach(function(id){if(mandatory.has(id)){if(selected.has(id)){pending.push({id:id,includeMembers:false})}return}if(!selected.has(id)){excluded.add(id);return}pending.push({id:id,includeMembers:false})})}if(excluded.size){result[group.framework]=Array.from(excluded)}});return result};
const selectFrameworkAsset=function(g,framework,asset){if(selectionConflict(g,framework.id)){return false}if(curatedFrameworkIds(g,framework.id).indexOf(asset.id)!==-1){return false}let group=g.externalSelections.find(function(item){return item.framework===framework.id});if(group&&group.items.some(function(item){return item.id===asset.id})){return false}if(!group){group={framework:framework.id,items:[],roots:[]};g.externalSelections.push(group)}else{preserveRootlessSelection(group)}group.items.push({kind:asset.kind,id:asset.id,source:{repository:asset.source.repository,commit:asset.source.commit,path:asset.source.path}});return true};
const selectFrameworkAssetClosure=function(g,framework,asset){let added=0,blocked=null;const pending=[{id:asset.id,includeMembers:true,required:true}],expanded=new Set();while(pending.length&&!blocked){const next=pending.shift();const expansionKey=next.id+"|"+(next.includeMembers?"members":"structural")+"|"+(next.required?"required":"suggested");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);if(frameworkAssetExcluded(framework.id,next.id)){if(next.required){blocked=next.id}continue}const target=frameworkAsset(framework.id,next.id);if(!target){if(next.required){blocked=next.id}continue}if(!isFrameworkSelected(framework.id,next.id)){if(!selectFrameworkAsset(g,target.framework,target.asset)){if(next.required){blocked=next.id}continue}added++}const mandatory=new Set(assetMandatoryRequirementIds(target.asset));assetReachabilityIds(target.asset,next.includeMembers).forEach(function(id){pending.push({id:id,includeMembers:false,required:mandatory.has(id)})})}return {added:added,blocked:blocked}};
/* Presets may propose structural module roots, but they never receive authority
   to select every Agent or Skill merely because a module lists it as a member.
   Structural dependencies remain fail-closed; center exclusions remain final. */
const selectFrameworkAssetStructure=function(g,framework,asset){let added=0,blocked=null;const pending=[asset.id],expanded=new Set();while(pending.length&&!blocked){const id=pending.shift();if(expanded.has(id)){continue}expanded.add(id);if(frameworkAssetExcluded(framework.id,id)){blocked=id;continue}const target=frameworkAsset(framework.id,id);if(!target){blocked=id;continue}if(!isFrameworkSelected(framework.id,id)){if(!selectFrameworkAsset(g,target.framework,target.asset)){blocked=id;continue}added++}assetMandatoryRequirementIds(target.asset).forEach(function(required){pending.push(required)})}return {added:added,blocked:blocked}};
const frameworkSelectionRoots=function(group){return Array.isArray(group.roots)?group.roots.slice():[]};
const markFrameworkSelectionRoots=function(frameworkId,ids){const group=externalSelectionGroups().find(function(item){return item.framework===frameworkId});if(!group){return}const roots=new Set(frameworkSelectionRoots(group));ids.forEach(function(id){roots.add(id)});group.roots=group.items.map(function(item){return item.id}).filter(function(id){return roots.has(id)});if(Array.isArray(group.unattributedItems)){group.unattributedItems=group.unattributedItems.filter(function(id){return !roots.has(id)});if(!group.unattributedItems.length){delete group.unattributedItems}}};
/* Derived requirements are not independent administrator choices. Rebuild the
   retained set from the remaining explicit roots so removing one aggregate
   prunes orphaned transitive dependencies but preserves shared or separately
   selected requirements. Rootless imported items stay separately attributed
   as retained legacy intent rather than being relabelled as direct choices. */
const frameworkRequirementClosure=function(framework,rootIds,memberRootIds){const retained=new Set(rootIds),memberRoots=new Set(memberRootIds===undefined?rootIds:memberRootIds),pending=rootIds.map(function(id){return {id:id,includeMembers:memberRoots.has(id)}}),expanded=new Set();while(pending.length){const next=pending.shift();const expansionKey=next.id+"|"+(next.includeMembers?"members":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);const asset=framework.assets.find(function(candidate){return candidate.id===next.id});if(!asset){continue}assetReachabilityIds(asset,next.includeMembers).forEach(function(required){retained.add(required);pending.push({id:required,includeMembers:false})})}return retained};
const frameworkMandatoryClosure=function(framework,rootIds){const retained=new Set(rootIds),pending=rootIds.slice();while(pending.length){const id=pending.shift();const asset=framework.assets.find(function(candidate){return candidate.id===id});if(!asset){continue}assetMandatoryRequirementIds(asset).forEach(function(required){if(retained.has(required)){return}retained.add(required);pending.push(required)})}return retained};
/* A rootless imported record predates selection attribution. It stays closed and
   untouched until a center action supplies administrator authority. At that
   point derive roots only from the pinned relationship graph: prefer items no
   selected item relates to, then aggregate/declaration owners for cyclic
   module-member graphs, and conservatively retain every item only when neither
   form exists. The chosen center item is removed after this conversion. */
const attributeRootlessSelection=function(group,framework){if(Array.isArray(group.roots)){return false}const selected=new Set(group.items.map(function(item){return item.id})),related=new Set();group.items.forEach(function(item){const asset=framework.assets.find(function(candidate){return candidate.id===item.id});if(!asset){return}assetReachabilityIds(asset,true).forEach(function(id){if(selected.has(id)){related.add(id)}})});let roots=group.items.map(function(item){return item.id}).filter(function(id){return !related.has(id)});if(!roots.length){roots=group.items.filter(function(item){const asset=framework.assets.find(function(candidate){return candidate.id===item.id});return asset&&((asset.members&&asset.members.length)||(asset.riders&&asset.riders.length))}).map(function(item){return item.id})}if(!roots.length){roots=group.items.map(function(item){return item.id})}const reachable=frameworkRequirementClosure(framework,roots),unattributed=group.items.map(function(item){return item.id}).filter(function(id){return !reachable.has(id)});group.roots=roots;if(unattributed.length){group.unattributedItems=unattributed}else{delete group.unattributedItems}return true};
const removeFrameworkSelectionRoots=function(g,framework,rootIds){const group=g.externalSelections.find(function(item){return item.framework===framework.id});if(!group){return 0}const removedRoots=new Set(rootIds);const roots=frameworkSelectionRoots(group).filter(function(id){return !removedRoots.has(id)});const suggestionRoots=roots.filter(function(id){return !frameworkRootIsStructural(framework.id,id)});const retained=frameworkRequirementClosure(framework,roots,suggestionRoots);const unattributed=new Set(Array.isArray(group.unattributedItems)?group.unattributedItems:[]);unattributed.forEach(function(id){retained.add(id)});const before=group.items.length;group.items=group.items.filter(function(item){return retained.has(item.id)});rootIds.forEach(function(id){restoreSuggestedFrameworkRoot(framework.id,id)});if(!group.items.length){g.externalSelections=g.externalSelections.filter(function(item){return item.framework!==framework.id})}else{const retainedIds=new Set(group.items.map(function(item){return item.id}));const retainedRoots=new Set(roots);group.roots=group.items.map(function(item){return item.id}).filter(function(id){return retainedRoots.has(id)});if(Array.isArray(group.unattributedItems)){group.unattributedItems=group.unattributedItems.filter(function(id){return retainedIds.has(id)});if(!group.unattributedItems.length){delete group.unattributedItems}}}return before-group.items.length};
const removeFrameworkSelectionDependents=function(g,framework,id){const group=g.externalSelections.find(function(item){return item.framework===framework.id});if(!group){return 0}const removed=new Set(group.items.filter(function(item){return frameworkMandatoryClosure(framework,[item.id]).has(id)}).map(function(item){return item.id}));const before=group.items.length;group.items=group.items.filter(function(item){return !removed.has(item.id)});if(Array.isArray(group.roots)){group.roots=group.roots.filter(function(root){return !removed.has(root)})}if(Array.isArray(group.unattributedItems)){group.unattributedItems=group.unattributedItems.filter(function(item){return !removed.has(item)});if(!group.unattributedItems.length){delete group.unattributedItems}}if(!group.items.length){g.externalSelections=g.externalSelections.filter(function(item){return item.framework!==framework.id})}return before-group.items.length};
...
const toggleFrameworkSelection=function(key,origin){
  const parts=String(key).split("|"),found=frameworkAsset(parts[0],parts[2]);if(!found){return}
  const previous=structuredClone(state.policy),previousExclusions=structuredClone(state.frameworkExclusions),previousStructural=structuredClone(state.structuralFrameworkRoots),g=ensureGovernance();
  if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}
  const group=g.externalSelections.find(function(item){return item.framework===found.framework.id});
  if(group&&group.items.some(function(item){return item.id===found.asset.id})){
    const attributedLegacy=origin==="center"&&attributeRootlessSelection(group,found.framework);
    const roots=frameworkSelectionRoots(group),requiredByRoots=roots.filter(function(root){return frameworkMandatoryClosure(found.framework,[root]).has(found.asset.id)});
    if(origin==="center"){
      const wasEnterpriseComplete=manualCompositionChange(),removedByRoots=requiredByRoots.length?removeFrameworkSelectionRoots(g,found.framework,requiredByRoots):0,removedDependents=removeFrameworkSelectionDependents(g,found.framework,found.asset.id),rootsNamed=requiredByRoots.length?" ("+requiredByRoots.join(", ")+")":"";
      excludeFrameworkAsset(found.framework.id,found.asset.id);
      if(!commitPolicy(previous,"Center deselected "+found.asset.id+"; removed "+requiredByRoots.length+" dependent root(s)"+rootsNamed+" and "+(removedByRoots+removedDependents)+" root or required component(s) from the exported policy."+(attributedLegacy?" Imported rootless intent was attributed from its pinned closure before applying this override.":"")+" Left composition helpers will not restore it until you re-select it in the center."+compositionBreakNotice(wasEnterpriseComplete))){state.frameworkExclusions=previousExclusions;state.structuralFrameworkRoots=previousStructural;render()}
      return
    }
    if(roots.indexOf(found.asset.id)===-1){
      const unattributed=Array.isArray(group.unattributedItems)&&group.unattributedItems.indexOf(found.asset.id)!==-1;
      if(unattributed){announce(found.asset.id+" came from an imported rootless policy, so its direct selection provenance is unknown. Use the center inventory to remove it and every item that requires it.",true);render();return}
      announce(found.asset.id+" cannot be deselected from the left composition helper while "+(requiredByRoots[0]||"another explicit root")+" requires it. Use the center inventory to override the composed selection.",true);render();return
    }
    const remainingRoots=roots.filter(function(root){return root!==found.asset.id}),retainedByRoot=remainingRoots.find(function(root){return frameworkRequirementClosure(found.framework,[root]).has(found.asset.id)}),wasEnterpriseComplete=manualCompositionChange(),removed=removeFrameworkSelectionRoots(g,found.framework,[found.asset.id]),related=found.asset.dependencies&&found.asset.dependencies.length||found.asset.members&&found.asset.members.length,relation=related?" dependency-closed component(s)":" selection and unshared rider(s)",message=retainedByRoot?"Removed the explicit root "+found.asset.id+", but the component remains selected because "+retainedByRoot+" requires it. Use the center inventory to remove the shared component and every dependent root.":"Deselected "+found.asset.id+"; removed "+removed+relation+" from the exported policy."+compositionBreakNotice(wasEnterpriseComplete);
    if(!commitPolicy(previous,message)){state.structuralFrameworkRoots=previousStructural;render()}return
  }
  const conflict=selectionConflict(g,found.framework.id);
  ...
  const previousPreset=activePreset,wasEnterpriseComplete=manualCompositionChange();
  if(origin==="center"){restoreFrameworkAsset(found.framework.id,found.asset.id)}
  const selected=selectFrameworkAssetClosure(g,found.framework,found.asset);
  if(selected.blocked){
    const blockedByCenter=frameworkAssetExcluded(found.framework.id,selected.blocked);
    state.policy=previous;state.frameworkExclusions=previousExclusions;state.structuralFrameworkRoots=previousStructural;activePreset=previousPreset;
    announce(blockedByCenter?"Selection blocked: required component "+selected.blocked+" remains excluded by your center-panel choice. Re-select it in the center before composing this item; nothing changed.":"Selection blocked: required component "+selected.blocked+" cannot be authored alongside its existing curation or framework state; nothing changed.",true);render();return
  }
  markFrameworkSelectionRoots(found.framework.id,[found.asset.id]);restoreSuggestedFrameworkRoot(found.framework.id,found.asset.id);
  const requirements=frameworkRequirementClosure(found.framework,[found.asset.id]);requirements.delete(found.asset.id);
  const related=found.asset.dependencies&&found.asset.dependencies.length||found.asset.members&&found.asset.members.length,relation=related?" required component(s)":" declared rider(s)";
  if(!commitPolicy(previous,"Selected "+found.asset.id+" with "+requirements.size+relation+": requested intent recorded with pinned sources."+compositionBreakNotice(wasEnterpriseComplete))){state.frameworkExclusions=previousExclusions;state.structuralFrameworkRoots=previousStructural;render()}
};
...
const frameworkInventoryRow=function(framework,asset,attributes){
  ...
  return row(framework.id+" / "+asset.kind+": "+asset.id,
    purpose+" By default, "+framework.id+" installs and runs it; AIH records the selection with its pinned source."+(asset.dependencies&&asset.dependencies.length?" Requires "+asset.dependencies.length+" module component(s): "+asset.dependencies.join(", ")+".":"")+(asset.members&&asset.members.length?" Contains "+asset.members.length+" selectable Skill, Agent, or baseline member(s): "+asset.members.join(", ")+".":"")+(asset.riders&&asset.riders.length?" Declares "+asset.riders.length+" rider(s): "+asset.riders.join(", ")+".":""),
    ...
    "Owned by "+framework.repository+" at "+framework.commit+", source "+asset.source.path+"."+(asset.dependencies&&asset.dependencies.length?" Required module dependency closure: "+asset.dependencies.join(", ")+".":"")+(asset.members&&asset.members.length?" Included selectable members: "+asset.members.join(", ")+".":"")+(asset.riders&&asset.riders.length?" Also brings in "+asset.riders.join(", ")+".":"")+" Evidence: "+evidenceCommand(framework,asset)+(asset.vet?" Already vetted at this pin by "+asset.vet.analyzers.map(function(a){return a.name+" "+a.version}).join(", ")+"; tree "+asset.vet.treeSha256.slice(0,12)+".":"")+vetNote(asset.vet)+(fulfillment?" "+fulfillment:""),
...
/* A gated AIH-owned row states ECC's declaration and AIH's own runtime identity
   separately: printing ECC's transport and then AIH's stdio-projector gate in
   one breath read as one contradictory sentence about one identity. */
const eccMcpInventoryRow=function(item){const approval=item.owner==="ecc";const hasAihControl=model.catalog.mcp.some(function(control){return control.id===item.id});const gate=aihMcpRequestGateEntry(item.id);const aihAvailabilityOnly=item.owner==="aih"&&!hasAihControl;if(aihAvailabilityOnly&&!gate){throw new Error("AIH-owned MCP declaration "+item.id+" is in no catalog partition: the AIH catalog must describe it as a control, non-projectable, or unavailable")}const requested=aihAvailabilityOnly&&isAihMcpRequested(item.id);const runtime=gate&&gate.configuredIdentity?"Configured runtime identity: "+gate.configuredIdentity+". "+gate.reason:gate?gate.reason:"";const detail=item.description+" ECC's pinned declaration: transport "+item.transport+", supply "+item.supply+", addability "+item.addability+". This row does not install, contact, scan, attest, or claim reachability."+(aihAvailabilityOnly?" AIH runtime identity: "+gate.transport+" transport. "+runtime+" Selecting this row records requested intent in governance.aihMcpRequests and grants no installation, evidence, approval, projection, activation, or reachability.":"");return row("ECC MCP: "+item.id,detail,approval?"Approval authoring available — not installed":hasAihControl?"Available — represented by the shared AIH control":(requested?"Requested intent — ":"Selectable — ")+aihMcpGateLabel(item.id),approval?"approval":requested?"requested":"external",approval?'<button type="button" class="btn sm" data-ecc-mcp-approval="'+esc(item.id)+'">Open approval</button>':aihAvailabilityOnly?tick("data-mcp-request",item.id,requested,item.id):"",approval?"ECC declares this at its pinned source; AIH records only a separately authored approval.":aihAvailabilityOnly?(requested?requestProvenance(item.id)+" ":"")+aihMcpRequestRoute(item.id):"ECC assigns this entry to AIH. This source view does not create an AIH control.",item.id,undefined,' data-ecc-mcp-availability="'+esc(item.id)+'" data-ecc-mcp-provenance="only"',approval?"":aihAvailabilityOnly&&gate.configuredIdentity?"AIH evidence required":"No AIH projector")};
const aihMcpInventoryRows=function(g){const controls=model.catalog.mcp.map(function(item){const existing=g.catalog.reviewed.find(function(c){return c.id===item.id});const conditional=item.availability==="web-target";const status=existing?candidateStatus(existing):[conditional?"Conditional — web target only":"Disabled","pending"];const declaration=model.catalog.eccMcpInventory.find(function(candidate){return candidate.owner==="aih"&&candidate.id===item.id});const detail=item.description+(conditional?" This control is offered for explicit authoring but profiles do not preselect it; runtime requires a detected web target.":"")+(declaration?" ECC also declares this identity with transport "+declaration.transport+", supply "+declaration.supply+", and addability "+declaration.addability+".":"")+activationTargetText(item.id);const provenance=controlProvenance(item.id)+(declaration?(controlProvenance(item.id)?" ":"")+"ECC source provenance is preserved on this shared row; it does not create a second control.":"");const attributes=declaration?' data-ecc-mcp-availability="'+esc(item.id)+'" data-ecc-mcp-provenance="shared"':"";const labels=[declaration?"Also declared by ECC":"",conditional?"Web target only":""].filter(Boolean).join(" · ");return row(item.id,detail,status[0],status[1],tick("data-reviewed",item.id,Boolean(existing),item.id),provenance,undefined,undefined,attributes,labels)});const availabilityOnly=model.catalog.eccMcpInventory.filter(function(item){return item.owner==="aih"&&!model.catalog.mcp.some(function(control){return control.id===item.id})}).map(eccMcpInventoryRow);return controls.concat(availabilityOnly).join("")};
...
const renderEccSkillInventory=function(){const framework=eccFramework(),host=byId("ecc-skill-rows");if(!framework){throw new Error("pinned ECC framework catalog is missing")}host.innerHTML=model.catalog.eccSkills.map(function(skill){const asset=framework.assets.find(function(candidate){return candidate.id==="skill:"+skill.id});if(!skill.governable||!asset){throw new Error("ECC skill is absent from the selectable policy catalog: "+skill.id)}return frameworkInventoryRow(framework,asset,' data-ecc-skill-availability="'+esc(skill.id)+'"')}).join("")};
...
const compositionClaimIds=function(part){const claimed=Array.isArray(state.compositionClaims[part.id])?state.compositionClaims[part.id]:[];const group=externalSelectionGroups().find(function(item){return item.framework===model.catalog.enterpriseComposition.framework});const roots=new Set(group?frameworkSelectionRoots(group):[]);return claimed.filter(function(id){return roots.has(id)})};
...
const ROW_STATES=["requested","pending","blocked","external","approval"];
let planeFilter="all";
/* The owner ticker. One surface at a time, so an administrator can look at what
   AIH owns without ECC's 411 exact baseline subjects in the way.
...
const PRESETS=[["custom","Custom","Start empty, then select only the components you intend to request."],
  ["vibe","Vibe","Selects every individually governable component from the active framework and every always-available AIH control; target-conditional controls stay additive."],
...
const syncRail=function(){const framework=eccFramework();if(!framework){return}const chosen=selectedItems(framework.id).map(function(item){return item.id});document.querySelectorAll(".chip[data-framework-select]").forEach(function(chip){const id=String(chip.getAttribute("data-framework-select")).split("|")[2];chip.setAttribute("aria-pressed",chosen.indexOf(id)===-1?"false":"true")});const posture=byId("rail-posture");if(posture){posture.textContent=(activePreset||"custom").replace(/^./,function(letter){return letter.toUpperCase()})}const preset=byId("preset-select");if(preset&&preset.value!==activePreset){preset.value=activePreset}const composition=byId("rail-composition-note");if(composition){const core=model.catalog.enterpriseComposition.parts.filter(function(part){return part.selection==="composed"});const selected=core.reduce(function(total,part){return total+partSelectedCount(part)},0);const total=core.reduce(function(count,part){return count+part.componentIds.length},0);composition.textContent=selected===0?"ECC Core not selected. Choose Enterprise for Core, or select components manually.":selected===total?"ECC Core complete: "+selected+" of "+total+" selected.":"Warning: ECC Core incomplete: "+selected+" of "+total+" selected; this composition does not match Enterprise."}
...
const paintShell=function(){const totals={requested:0,pending:0,blocked:0,external:0,approval:0,availability:0};let shown=0,total=0,vetBlocked=0;
  ...
  ROW_STATES.filter(function(s){return s!=="approval"}).forEach(function(s){const node=byId(s==="requested"?"t-req":s==="pending"?"t-wait":s==="blocked"?"t-blk":"t-ext");if(node){node.textContent=String(totals[s])}});
...
const assetSourcePaths=function(asset){const paths=asset&&Array.isArray(asset.sourcePaths)?asset.sourcePaths.filter(function(path){return typeof path==="string"&&path.length>0}):[];return paths.length?paths:[asset.source.path]};
...
const paintDrawer=function(id){
  ...
  if(item.externalMcp){const externalMcp=item.externalMcp;
    const approval=externalMcp.owner==="ecc";const hasAihControl=model.catalog.mcp.some(function(control){return control.id===externalMcp.id});const unavailable=model.catalog.unavailableMcp.find(function(candidate){return candidate.id===externalMcp.id});const gate=externalMcp.owner==="aih"&&!hasAihControl?aihMcpRequestGateEntry(externalMcp.id):undefined;const launch=externalMcp.url||([externalMcp.command].concat(externalMcp.args||[]).filter(Boolean).join(" "))||"manual configuration";const credentials=externalMcp.credentialRequirement.variables.length?externalMcp.credentialRequirement.variables.join(", "):"none declared";const provenance=model.catalog.eccMcpProvenance;const audit=auditDetails('<div class="cap">Provenance</div><div class="kv">'+kv("Repository",esc(provenance.repository))+kv("Pinned commit",scrollCode(provenance.commit))+kv("Source definition",esc(provenance.path))+kv("Content SHA-256",scrollCode(model.catalog.eccMcpApproval.sourceContentSha256))+(gate?kv("AIH runtime transport",esc(gate.transport)):"")+(unavailable?kv("Configured runtime identity",scrollCode(unavailable.configuredIdentity)):"")+kv("Credential variables",esc(credentials))+kv("Supply",esc(externalMcp.supply))+kv("Addability",esc(externalMcp.addability))+'</div><p class="note">This source-locked declaration is not installed, contacted, scanned, attested, or represented as reachable.'+(approval?"":hasAihControl?" A separate AIH control row exists for this id.":unavailable?" "+esc(unavailable.reason):" This Core build ships no AIH policy control for this id; a recorded request does not create one.")+'</p>');host.innerHTML='<div class="dhead"><h2>'+esc(item.id)+'</h2>'+close+'</div><div class="badges"><span class="b ext">ECC source metadata</span><span class="b">MCP</span></div><div class="cap">Component overview</div><p class="note">'+esc(externalMcp.description)+'</p><div class="kv">'+kv("Allowed tools / scope",esc(externalMcp.description))+kv("Transport",esc(externalMcp.transport))+kv("Declared launch",scrollCode(launch))+'</div><p class="note"><b>Usage context:</b> '+esc("Declared for "+externalMcp.transport+" use through "+launch+".")+'</p>'+componentStatus(false,approval?"An administrator must review and author approval for this exact pinned declaration.":gate?"This entry has no separate scanner verdict. Selecting it records requested intent over AIH's own runtime identity in governance.aihMcpRequests — never over ECC's declaration — and grants no installation, evidence, approval, projection, activation, or reachability. "+gate.reason:"This entry has no separate scanner verdict. A separate AIH control row exists for this id.")+audit+(approval?'<div class="brow"><button type="button" class="btn sm" data-ecc-mcp-approval="'+esc(externalMcp.id)+'">Open approval authoring</button></div>':"");return}
...
const spotItems=function(){const list=[];
  ...
  model.catalog.eccSkills.forEach(function(item){list.push({id:"ecc / skill: skill:"+item.id,group:"ECC skills"})});
  model.catalog.frameworks.forEach(function(framework){framework.assets.forEach(function(asset){
...
const applyPreset=function(value){state.compositionClaims={};if(value==="custom"){const previous=structuredClone(state.policy);activePreset="custom";state.policy=structuredClone(model.initialPolicy);state.editing=null;state.frameworkExclusions=inferFrameworkExclusions(state.policy);commitPolicy(previous,"Custom composition started empty. Select only the components you intend to request.");return}activePreset=value;if(value==="vibe"){composeVibeProfile();return}if(value==="enterprise"){composeEnterpriseProfile()}};
byId("posture").addEventListener("change",function(event){const value=event.target.value;const supported=state.policy.governance&&Array.isArray(state.policy.governance.supportedClis)?state.policy.governance.supportedClis:[];if(value==="enterprise"&&!supported.length){announce("Enterprise posture was not applied. Select at least one Allowed CLI first, or choose the Enterprise preset to explicitly sanction every supported CLI and compose Core.",true);render();return}const previous=structuredClone(state.policy);activePreset="custom";state.policy.minimumPosture=value;commitPolicy(previous,"Posture changed without modifying selections.")});
...
document.addEventListener("click",function(event){const helpButton=event.target.closest("[data-tooltip-button]");if(helpButton){openTooltip(helpButton);return}const target=event.target.closest("[data-reviewed]");if(target){toggleReviewed(target.getAttribute("data-reviewed"));return}const request=event.target.closest("[data-mcp-request]");if(request){toggleMcpRequest(request.getAttribute("data-mcp-request"));return}closeTooltips()});
...
let centerSelectionContextVersion=0;
const restoreCenterSelectionContext=function(key,scrollX,scrollY,anchorTop){const version=++centerSelectionContextVersion;const replacement=function(){return [].slice.call(document.querySelectorAll("[data-framework-select]")).find(function(candidate){return candidate.getAttribute("data-framework-select")===key&&!candidate.closest(".rail")})};const control=replacement();if(control){control.focus({preventScroll:true})}const restorePosition=function(){if(version!==centerSelectionContextVersion){return}const current=replacement();if(!current){return}const delta=current.getBoundingClientRect().top-anchorTop;window.scrollTo(scrollX,Math.max(0,window.scrollY+delta))};window.scrollTo(scrollX,scrollY);restorePosition();if(typeof window.requestAnimationFrame==="function"){window.requestAnimationFrame(function(){restorePosition();window.requestAnimationFrame(restorePosition)})}};
document.addEventListener("click",function(event){const button=event.target.closest&&event.target.closest("[data-framework-select]");if(!button){return}const key=button.getAttribute("data-framework-select");const fromRail=Boolean(button.closest(".rail"));const scrollX=window.scrollX,scrollY=window.scrollY,anchorTop=button.getBoundingClientRect().top;if(fromRail){revealRailSelectionGroup(key)}toggleFrameworkSelection(key,fromRail?"rail":"center");if(!fromRail){restoreCenterSelectionContext(key,scrollX,scrollY,anchorTop)}});
...
document.addEventListener("click",function(event){const button=event.target.closest&&event.target.closest("[data-composition-add]");if(!button||button.disabled){return}const part=model.catalog.enterpriseComposition.parts.find(function(item){return item.id===button.getAttribute("data-composition-add")});if(!part){return}const previous=structuredClone(state.policy);const previousClaims=structuredClone(state.compositionClaims);const previousStructural=structuredClone(state.structuralFrameworkRoots);const previousPreset=activePreset;const wasEnterpriseComplete=manualCompositionChange();const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const framework=eccFramework();if(!framework){activePreset=previousPreset;announce("ECC composition is unavailable because its pinned framework catalog is missing.",true);render();return}const claimed=compositionClaimIds(part);if(claimed.length){const removed=removeFrameworkSelectionRoots(g,framework,claimed);delete state.compositionClaims[part.id];if(!commitPolicy(previous,"Removed "+part.label+": "+removed+" aggregate-owned root and unshared required component(s) are no longer requested."+compositionBreakNotice(wasEnterpriseComplete))){state.compositionClaims=previousClaims;state.structuralFrameworkRoots=previousStructural;activePreset=previousPreset;render()}return}const group=g.externalSelections.find(function(item){return item.framework===framework.id});const priorRoots=new Set(group?frameworkSelectionRoots(group):[]);const result=selectCompositionPart(g,part);if(result.blocked){state.policy=previous;state.structuralFrameworkRoots=previousStructural;activePreset=previousPreset;announce("Adding "+part.label+" was blocked because "+result.blocked+" cannot be selected with its required closure; nothing changed.",true);render();return}markFrameworkSelectionRoots(framework.id,part.componentIds);part.componentIds.forEach(function(id){restoreSuggestedFrameworkRoot(framework.id,id)});state.compositionClaims[part.id]=part.componentIds.filter(function(id){return !priorRoots.has(id)});if(!commitPolicy(previous,"Added "+part.label+": "+result.added+" root and required component(s) selected as requested intent with their pinned sources. Audit evidence is still owed for each."+compositionBreakNotice(wasEnterpriseComplete))){state.compositionClaims=previousClaims;state.structuralFrameworkRoots=previousStructural;activePreset=previousPreset;render()}});
...
byId("import-policy").addEventListener("click",function(){byId("policy-file").click()});byId("policy-file").addEventListener("change",function(event){readFile(event.target,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const prepared=preparePolicyImport(value,function(policy){return schemaErrors(model.schema,policy,"").concat(policySemantics(policy),managedMcpPolicySemantics(policy),aihMcpRequestSemantics(policy),selectionDependencySemantics(policy),selectionAttributionSemantics(policy),kiroTransportSemantics(policy),policyTextSemantics(policy))});state.policy=prepared.policy;state.compositionClaims={};state.structuralFrameworkRoots=inferStructuralFrameworkRoots(state.policy);state.frameworkExclusions=inferFrameworkExclusions(state.policy,state.structuralFrameworkRoots);activePreset="custom";announce(prepared.message);render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}})});
...
byId("download").addEventListener("click",function(){const problems=policyProblems();if(problems.length){announce("Download blocked: "+problems.slice(0,3).join("; "),true);return}const field=byId("policy-download-name"),filename=field?field.value.trim():"aih-org-policy.json";if(!/^[A-Za-z0-9][A-Za-z0-9._-]{0,126}\.json$/.test(filename)){announce("Download blocked: Use a JSON filename without folders, spaces, or hidden characters.",true);return}const blob=new Blob([policyText()],{type:"application/json"});const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=filename;link.click();URL.revokeObjectURL(url);announce("Policy download started. Validate this file with: aih policy validate <target-root> --policy "+filename)});
...
paintHosts();
document.addEventListener("click",function(event){const button=event.target.closest&&event.target.closest("[data-sanctioned-cli]");if(!button){return}const cli=button.getAttribute("data-sanctioned-cli");const hosts=model.catalog.hosts||[];if(!hosts.some(function(host){return host.id===cli})){return}const previous=structuredClone(state.policy);const g=ensureGovernance();const selected=new Set(Array.isArray(g.supportedClis)?g.supportedClis:[]);if(selected.has(cli)){selected.delete(cli)}else{selected.add(cli)}const ordered=hosts.map(function(host){return host.id}).filter(function(id){return selected.has(id)});if(ordered.length){g.supportedClis=ordered}else{delete g.supportedClis}const scopeProblem=rebindReviewedActivationTargets(g);if(scopeProblem){state.policy=previous;announce("Policy change rejected: "+scopeProblem+". Remove that control before removing its last projectable sanctioned CLI.",true);render();return}commitPolicy(previous,ordered.length?"Supported CLI allow-list updated: "+ordered.join(", ")+". Reviewed activation targets were rebound to their exact sanctioned projector intersections; unsanctioned selected or detected CLIs are refused by the engine.":"Supported CLI allow-list cleared without broadening existing activation targets. Vibe permits an omitted list; Enterprise requires an explicit list.")});
...
document.addEventListener("click",function(event){if(!event.target.closest||!event.target.closest("#clear-policy")){return}
  state.policy=structuredClone(model.initialPolicy);state.editing=null;state.compositionClaims={};state.structuralFrameworkRoots=inferStructuralFrameworkRoots(state.policy);state.frameworkExclusions=inferFrameworkExclusions(state.policy,state.structuralFrameworkRoots);activePreset="custom";
...
state.structuralFrameworkRoots=inferStructuralFrameworkRoots(state.policy);
state.frameworkExclusions=inferFrameworkExclusions(state.policy,state.structuralFrameworkRoots);
buildRail();
render();
</script>
<script>
...
const browserArgumentErrors=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const sources=[];const catalog=governanceValue.catalog||{};["reviewed","custom"].forEach(function(collection){(catalog[collection]||[]).forEach(function(candidate,index){sources.push({source:candidate&&candidate.source,path:"policy.governance.catalog."+collection+"["+index+"]"})})});const approvals=governanceValue.authority&&governanceValue.authority.approvals||[];approvals.forEach(function(approval,index){sources.push({source:approval&&approval.source,path:"policy.governance.authority.approvals["+index+"]"})});sources.forEach(function(entry){const source=entry.source;if(source&&(source.type==="package"||source.type==="stdio")&&!safeBrowserHttpsOrigin(source.registry)){errors.push(validationError(entry.path+".source.registry","must be an exact HTTPS origin"))}if(source&&source.type==="command"&&Array.isArray(source.args)){source.args.forEach(function(argument,index){if(!safeBrowserArgument(argument)){errors.push(validationError(entry.path+".source.args["+index+"]","must be a safe relative argument or exact HTTPS registry/index origin"))}})}});return errors};
  ...
  const browserSemanticErrors=function(policy){return policySemantics(policy).filter(function(error){return !/\.source\.args\[\d+\]: must be a safe relative argument/.test(error)}).concat(managedMcpPolicySemantics(policy),aihMcpRequestSemantics(policy),selectionDependencySemantics(policy),selectionAttributionSemantics(policy),kiroTransportSemantics(policy),browserArgumentErrors(policy),browserRootErrors(policy))};
  ...
  byId("policy-file").addEventListener("change",function(event){event.stopImmediatePropagation();readInput(event.currentTarget,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const prepared=preparePolicyImport(value,function(policy){return schemaErrors(model.schema,policy,"").concat(browserSemanticErrors(policy),policyTextSemantics(policy))});state.policy=prepared.policy;state.editing=null;state.compositionClaims={};state.structuralFrameworkRoots=inferStructuralFrameworkRoots(state.policy);state.frameworkExclusions=inferFrameworkExclusions(state.policy,state.structuralFrameworkRoots);activePreset="custom";announce(prepared.message);render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}},true);
  },true);
  const exportNarration=function(){const g=governance();const controls=g.activations.filter(function(item){return item.state==="active"}).length;const external=externalSelectionGroups().reduce(function(total,group){return total+group.items.length},0);return (controls+external+aihMcpRequests().length)+" requested item(s), 0 effective in this browser; import into a target repository for engine evaluation."};
  const narrateReport=function(){const report=byId("report-preview");report.value=report.value+"\n\nExport summary: "+exportNarration()};
  const runValidation=function(event,mode){event.preventDefault();event.stopImmediatePropagation();const problems=browserProblems();if(problems.length){announce((mode==="download"?"Download blocked: ":mode==="export"?"Export blocked: ":"Schema and policy-grammar validation failed: ")+problems.slice(0,3).join("; "),true);return false}if(mode==="download"){const field=byId("policy-download-name"),filename=field?field.value.trim():"aih-org-policy.json";if(!/^[A-Za-z0-9][A-Za-z0-9._-]{0,126}\.json$/.test(filename)){announce("Download blocked: Use a JSON filename without folders, spaces, or hidden characters.",true);return false}const blob=new Blob([policyText()],{type:"application/json"});const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=filename;link.click();URL.revokeObjectURL(url);announce("Policy download started. Validate this file with: aih policy validate <target-root> --policy "+filename+". "+exportNarration())}else if(mode==="export"){renderPreview();narrateReport();openSheet();announce("Policy export preview refreshed from the actual policy schema and grammar. "+exportNarration())}else{announce("Schema and policy-grammar validation passed. Authority, scans, projection, and effective state require the AIH engine in a target repository.");renderPreview()}return true};
...
byId("copy-approvals").addEventListener("click",function(event){event.preventDefault();event.stopImmediatePropagation();if(!(state.receipt&&Array.isArray(state.receipt.approvals))){return}const previous=structuredClone(state.policy);ensureGovernance().authority.approvals=structuredClone(state.receipt.approvals);if(browserProblems().length){state.policy=previous;announce("Approval preservation rejected: imported subjects do not satisfy the actual policy grammar.",true);render();return}announce("Approval subjects preserved in governance.authority.approvals; no signature or effective-approval claim is made.");render()},true);
  ...
  const prepareWorkspacePolicy=function(value){return preparePolicyImport(value,function(policy){return schemaErrors(model.schema,policy,"").concat(browserSemanticErrors(policy),policyTextSemantics(policy))})};
  window.__aihPolicyWorkbenchSession={snapshotPolicy:function(){return structuredClone(state.policy)},validatePolicy:prepareWorkspacePolicy,restorePolicy:function(value){const prepared=prepareWorkspacePolicy(value);state.policy=prepared.policy;state.editing=null;state.compositionClaims={};state.structuralFrameworkRoots=inferStructuralFrameworkRoots(state.policy);state.frameworkExclusions=inferFrameworkExclusions(state.policy,state.structuralFrameworkRoots);activePreset="custom";render();return prepared.message}};
...
importBar.innerHTML='<div class="cap">Team policy file <span id="import-tip-slot"></span></div><div class="form-grid"><label>Policy download filename<input id="policy-download-name" value="aih-org-policy.json" maxlength="132" autocomplete="off" spellcheck="false"></label><div><p class="help" id="policy-file-help">Use one safe JSON filename per project or team. The browser chooses the download folder; move the file into an administrator-controlled policy folder when required.</p><pre class="mono" id="policy-file-command">aih policy validate &lt;target-root&gt; --policy aih-org-policy.json</pre></div></div><div class="cap">Import files</div><div class="brow" id="import-actions"></div>';
imports.append(importBar);
byId("import-tip-slot").append(infoTip("The policy filename selects no authority by itself. Importing a policy replaces the current document after validation. Scanner evidence files remain separate records; evidence and decisions imported here are preserved for preflight inspection only and never become effective in this browser.","About team policy files and imports"));
byId("import-actions").append(byId("import-policy"),byId("import-evidence"),byId("import-decision"));
var policyDownloadName=byId("policy-download-name"),policyFileHelp=byId("policy-file-help"),policyFileCommand=byId("policy-file-command");
var syncPolicyFilename=function(){var filename=policyDownloadName.value.trim(),safe=/^[A-Za-z0-9][A-Za-z0-9._-]{0,126}\.json$/.test(filename);policyDownloadName.setAttribute("aria-invalid",safe?"false":"true");policyFileHelp.textContent=safe?"Use one safe JSON filename per project or team. The browser chooses the download folder; move the file into an administrator-controlled policy folder when required.":"Use a JSON filename without folders, spaces, or hidden characters.";policyFileCommand.textContent=safe?"aih policy validate <target-root> --policy "+filename:"aih policy validate <target-root> --policy <safe-policy-file.json>"};
policyDownloadName.addEventListener("input",syncPolicyFilename);syncPolicyFilename();
...
presetSel.addEventListener("change",function(){applyPreset(presetSel.value);syncPreset()});
...
var openPop=null;
var insideOpenPopClicks=new WeakSet();
...
var hostActions=document.createElement("div");hostActions.className="brow";
var hostSelectAll=document.createElement("button");hostSelectAll.type="button";hostSelectAll.className="btn sm";hostSelectAll.dataset.supportedCliAction="select-all";hostSelectAll.textContent="Select all";
var hostDone=document.createElement("button");hostDone.type="button";hostDone.className="btn sm primary";hostDone.dataset.supportedCliAction="done";hostDone.textContent="Done";
hostActions.append(hostSelectAll,hostDone);
hostSelectAll.addEventListener("click",function(){var ids=(model.catalog.hosts||[]).map(function(host){return host.id});var previous=structuredClone(state.policy),g=ensureGovernance();g.supportedClis=ids;rebindReviewedActivationTargets(g);commitPolicy(previous,"All "+ids.length+" registry CLIs explicitly sanctioned. Reviewed activation targets now match their exact sanctioned projector intersections; unsanctioned selected or detected CLIs are refused by the engine.")});
hostDone.addEventListener("click",function(){closePop()});
makePop("Allowed CLI",IC.hosts,[hostsChips,hostActions,hostCountLine,byId("rail-host-note")],true,byId("preset-poplist"));
hostsSect.remove();
...
document.addEventListener("click",function(e){if(openPop&&(openPop.pop.contains(e.target)||openPop.row.contains(e.target))){insideOpenPopClicks.add(e)}},true);
document.addEventListener("click",function(e){if(!openPop){return}var path=typeof e.composedPath==="function"?e.composedPath():[];var originatedInside=insideOpenPopClicks.has(e)||path.indexOf(openPop.pop)!==-1||path.indexOf(openPop.row)!==-1;if(!originatedInside&&!openPop.pop.contains(e.target)&&!openPop.row.contains(e.target)){closePop()}});
...
var syncCompact=function(){pops.forEach(function(p){var b=p.row.querySelector(".selcount");if(!p.countable){return}var total=p.pop.querySelectorAll("[aria-pressed]").length;var sel=p.pop.querySelectorAll('[aria-pressed="true"]').length;var t=sel+" / "+total;if(b&&b.textContent!==t){b.textContent=t}});syncPreset();syncPosture()};
