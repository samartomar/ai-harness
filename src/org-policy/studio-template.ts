...
const adoptionRouteText=function(route){if(route.kind==="workbench-row"){return "Existing Workbench row: "+route.candidate}if(route.kind==="ecc-mcp-approval"){return "ECC MCP approval for "+route.id+", then manual "+route.addability+" configuration"}if(route.kind==="aih-ecc-profile-lifecycle"){return "AIH ECC profile lifecycle: "+route.command}return "No route"};
const renderAdoptionRecipe=function(){const recipe=model.adoptionRecipe;byId("adoption-recipe-roles").innerHTML=recipe.roles.map(function(role){const usage=role.usage.kind==="mcp-server-event"?"MCP server event: "+role.usage.serverId:"Usage signal: none captured";return '<article class="adoption-role" data-adoption-role="'+esc(role.id)+'"><strong>'+esc(role.label)+'</strong><p>'+esc(role.guidance)+'</p><p class="adoption-route">'+esc(adoptionRouteText(role.route))+". "+esc(usage)+".</p></article>"}).join("")};
