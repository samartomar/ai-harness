...
const selectFrameworkAssetClosure=function(g,framework,asset){let added=0;let blocked=null;const pending=[asset.id],visited=new Set();while(pending.length&&!blocked){const id=pending.shift();if(visited.has(id)){continue}visited.add(id);const target=frameworkAsset(framework.id,id);if(!target){blocked=id;break}if(!isFrameworkSelected(framework.id,id)){if(!selectFrameworkAsset(g,target.framework,target.asset)){blocked=id;break}added++}assetRequirementIds(target.asset).forEach(function(required){if(!visited.has(required)){pending.push(required)}})}return {added:added,blocked:blocked}};
...
const frameworkRequirementClosure=function(framework,rootIds){const retained=new Set(rootIds),pending=rootIds.slice();while(pending.length){const id=pending.shift();const asset=framework.assets.find(function(candidate){return candidate.id===id});if(!asset){continue}assetRequirementIds(asset).forEach(function(required){if(!retained.has(required)){retained.add(required);pending.push(required)}})}return retained};
