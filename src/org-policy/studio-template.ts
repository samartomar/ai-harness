...
const assetRequirementIds=function(asset){return Array.from(new Set([].concat(asset&&asset.riders||[],asset&&asset.dependencies||[])))};
...
const selectFrameworkAssetClosure=function(g,framework,asset){let added=0;let blocked=null;[asset.id].concat(assetRequirementIds(asset)).some(function(id){const target=frameworkAsset(framework.id,id);if(!target){blocked=id;return true}if(isFrameworkSelected(framework.id,id)){return false}if(!selectFrameworkAsset(g,target.framework,target.asset)){blocked=id;return true}added++;return false});return {added:added,blocked:blocked}};
...
const toggleFrameworkSelection=function(key){const parts=String(key).split("|");const found=frameworkAsset(parts[0],parts[2]);if(!found){return}const previous=structuredClone(state.policy);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const group=g.externalSelections.find(function(item){return item.framework===found.framework.id});if(group&&group.items.some(function(item){return item.id===found.asset.id})){const requiredBy=group.items.find(function(item){if(item.id===found.asset.id){return false}const asset=found.framework.assets.find(function(candidate){return candidate.id===item.id});return Boolean(asset&&assetRequirementIds(asset).indexOf(found.asset.id)!==-1)});if(requiredBy){announce(found.asset.id+" cannot be deselected while "+requiredBy.id+" requires it. Deselect the dependent selection first.",true);render();return}const wasEnterpriseComplete=manualCompositionChange();const removed=removeFrameworkSelectionRoots(g,found.framework,[found.asset.id]);const relation=found.asset.dependencies&&found.asset.dependencies.length?" dependency-closed component(s)":" selection and unshared rider(s)";commitPolicy(previous,"Deselected "+found.asset.id+"; removed "+removed+relation+" from the exported policy."+compositionBreakNotice(wasEnterpriseComplete));return}const conflict=selectionConflict(g,found.framework.id);
  ...
  const previousPreset=activePreset;const wasEnterpriseComplete=manualCompositionChange();const selected=selectFrameworkAssetClosure(g,found.framework,found.asset);if(selected.blocked){state.policy=previous;activePreset=previousPreset;announce("Selection blocked: required component "+selected.blocked+" cannot be authored alongside its existing curation or framework state; nothing changed.",true);render();return}markFrameworkSelectionRoots(found.framework.id,[found.asset.id]);const requirements=assetRequirementIds(found.asset);const relation=found.asset.dependencies&&found.asset.dependencies.length?" required component(s)":" declared rider(s)";commitPolicy(previous,"Selected "+found.asset.id+" with "+requirements.length+relation+": requested intent recorded with pinned sources."+compositionBreakNotice(wasEnterpriseComplete))};
...
const frameworkInventoryRow=function(framework,asset,attributes){
  ...
  return row(framework.id+" / "+asset.kind+": "+asset.id,
    purpose+" By default, "+framework.id+" installs and runs it; AIH records the selection with its pinned source."+(asset.dependencies&&asset.dependencies.length?" Requires "+asset.dependencies.length+" module component(s): "+asset.dependencies.join(", ")+".":"")+(asset.riders&&asset.riders.length?" Declares "+asset.riders.length+" rider(s): "+asset.riders.join(", ")+".":""),
