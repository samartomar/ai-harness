...
byId("import-decision").addEventListener("click",function(){byId("decision-file").click()});byId("decision-file").addEventListener("change",function(event){const input=event.target;const file=input.files&&input.files[0];if(!file){return}const selection=++decisionSelection;const prior=state.decision===null?null:structuredClone(state.decision);const reader=new FileReader();reader.onload=function(){if(selection!==decisionSelection){return}try{const value=JSON.parse(String(reader.result||""));const problems=decisionProblems(value);if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.decision=structuredClone(value);announce("Decision imported for inspection only: unverified and not effective.");renderDecision()}catch(error){state.decision=prior;renderDecision();announce("Decision import rejected: "+(error&&error.message?error.message:"strict decision JSON required"),true)}};reader.readAsText(file)});
