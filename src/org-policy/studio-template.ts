...
const selectionDependencySemantics=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}(governanceValue.externalSelections||[]).forEach(function(group,groupIndex){const framework=model.catalog.frameworks.find(function(item){return item.id===group.framework});if(!framework){return}const selected=new Set((group.items||[]).map(function(item){return item.id}));const roots=new Set();(group.roots||[]).forEach(function(root,rootIndex){if(roots.has(root)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].roots["+rootIndex+"]","duplicate external selection root "+root))}if(!selected.has(root)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].roots["+rootIndex+"]","selection root "+root+" is not present in items"))}roots.add(root)});(group.items||[]).forEach(function(item,itemIndex){const asset=framework.assets.find(function(candidate){return candidate.id===item.id});if(!asset){return}assetRequirementIds(asset).forEach(function(required){if(!selected.has(required)){errors.push(validationError("policy.governance.externalSelections["+groupIndex+"].items["+itemIndex+"]",item.id+" requires "+required+" in the same dependency-closed selection"))}})})});return errors};
...
const frameworkSelectionRoots=function(group){return Array.isArray(group.roots)?group.roots.slice():group.items.map(function(item){return item.id})};
const markFrameworkSelectionRoots=function(frameworkId,ids){const group=externalSelectionGroups().find(function(item){return item.framework===frameworkId});if(!group){return}const roots=new Set(frameworkSelectionRoots(group));ids.forEach(function(id){roots.add(id)});group.roots=group.items.map(function(item){return item.id}).filter(function(id){return roots.has(id)})};
/* Derived requirements are not independent administrator choices. Rebuild the
   ...
   selected requirements. Imported items are all marked explicit and are
   therefore retained conservatively instead of reinterpreting user intent. */
...
const toggleFrameworkSelection=function(key){const parts=String(key).split("|");const found=frameworkAsset(parts[0],parts[2]);if(!found){return}const previous=structuredClone(state.policy);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const group=g.externalSelections.find(function(item){return item.framework===found.framework.id});if(group&&group.items.some(function(item){return item.id===found.asset.id})){const requiredBy=group.items.find(function(item){if(item.id===found.asset.id){return false}const asset=found.framework.assets.find(function(candidate){return candidate.id===item.id});return Boolean(asset&&assetRequirementIds(asset).indexOf(found.asset.id)!==-1)});if(requiredBy){announce(found.asset.id+" cannot be deselected while "+requiredBy.id+" requires it. Deselect the dependent selection first.",true);render();return}const wasEnterpriseComplete=manualCompositionChange();const removed=removeFrameworkSelectionRoots(g,found.framework,[found.asset.id]);const related=found.asset.dependencies&&found.asset.dependencies.length||found.asset.members&&found.asset.members.length;const relation=related?" dependency-closed component(s)":" selection and unshared rider(s)";commitPolicy(previous,"Deselected "+found.asset.id+"; removed "+removed+relation+" from the exported policy."+compositionBreakNotice(wasEnterpriseComplete));return}const conflict=selectionConflict(g,found.framework.id);
...
document.addEventListener("click",function(event){const button=event.target.closest&&event.target.closest("[data-composition-add]");if(!button){return}const part=model.catalog.enterpriseComposition.parts.find(function(item){return item.id===button.getAttribute("data-composition-add")});if(!part){return}const previous=structuredClone(state.policy);const wasEnterpriseComplete=manualCompositionChange();const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const framework=eccFramework();if(!framework){announce("ECC composition is unavailable because its pinned framework catalog is missing.",true);return}if(partSelectedCount(part)===part.componentIds.length){const removed=removeFrameworkSelectionRoots(g,framework,part.componentIds);commitPolicy(previous,"Removed "+part.label+": "+removed+" root and unshared required component(s) are no longer requested."+compositionBreakNotice(wasEnterpriseComplete));return}const result=selectCompositionPart(g,part);if(result.blocked){state.policy=previous;announce("Adding "+part.label+" was blocked because "+result.blocked+" cannot be selected with its required closure; nothing changed.",true);render();return}markFrameworkSelectionRoots(framework.id,part.componentIds);commitPolicy(previous,"Added "+part.label+": "+result.added+" root and required component(s) selected as requested intent with their pinned sources. Audit evidence is still owed for each."+compositionBreakNotice(wasEnterpriseComplete))});
...
byId("import-policy").addEventListener("click",function(){byId("policy-file").click()});byId("policy-file").addEventListener("change",function(event){readFile(event.target,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const problems=schemaErrors(model.schema,value,"").concat(policySemantics(value),selectionDependencySemantics(value),kiroTransportSemantics(value),policyTextSemantics(value));if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.policy=value;activePreset="custom";announce("Policy imported without transformation after schema and policy-grammar validation.");render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}})});
...
document.addEventListener("click",function(event){if(!event.target.closest||!event.target.closest("#clear-policy")){return}
  state.policy=structuredClone(model.initialPolicy);state.editing=null;activePreset="custom";
