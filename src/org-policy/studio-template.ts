...
const policyProblems=function(){return schemaErrors(model.schema,state.policy,"").concat(policySemantics(state.policy),managedMcpPolicySemantics(state.policy),selectionDependencySemantics(state.policy),selectionAttributionSemantics(state.policy),kiroTransportSemantics(state.policy),policyTextSemantics(state.policy))};
...
const compactState=function(kind,status){if(kind==="requested"){return "Selected"}if(kind==="blocked"){return "Blocked"}if(kind==="approval"){return "Approval"}if(kind==="availability"){return status.indexOf("Unavailable")===0?"Awaiting":"Available"}if(kind==="pending"){return status.indexOf("Disabled")===0?"Disabled":"Awaiting"}if(kind==="external"){return status.indexOf("Selectable")===0?"Selectable":"External"}return "External"};
...
const eccMcpInventoryRow=function(item){const approval=item.owner==="ecc";const hasAihControl=model.catalog.mcp.some(function(control){return control.id===item.id});const unavailable=model.catalog.unavailableMcp.find(function(candidate){return candidate.id===item.id});const aihAvailabilityOnly=item.owner==="aih"&&!hasAihControl;const detail=item.description+" Transport "+item.transport+", supply "+item.supply+", addability "+item.addability+". This row does not install, contact, scan, attest, or claim reachability."+(unavailable?" Configured runtime identity: "+unavailable.configuredIdentity+". "+unavailable.reason:aihAvailabilityOnly?" This identity is runtime availability only and is not policy-projectable by the managed stdio projector.":"");return row("ECC MCP: "+item.id,detail,approval?"Approval authoring available — not installed":hasAihControl?"Available — represented by the shared AIH control":unavailable?"Unavailable — AIH evidence required":"Available — not policy-projectable",approval?"approval":"availability",approval?'<button type="button" class="btn sm" data-ecc-mcp-approval="'+esc(item.id)+'">Open approval</button>':"",approval?"ECC declares this at its pinned source; AIH records only a separately authored approval.":unavailable?unavailable.reason:aihAvailabilityOnly?"AIH runtime and ECC declare this identity, but the current policy projector cannot own it; it remains visible and unselectable.":"ECC assigns this entry to AIH. This source view does not create an AIH control.",item.id,undefined,' data-ecc-mcp-availability="'+esc(item.id)+'" data-ecc-mcp-provenance="only"',approval?"":unavailable?"Unavailable":"Runtime availability only")};
...
const paintDrawer=function(id){
  ...
  if(item.externalMcp){const externalMcp=item.externalMcp;
    const approval=externalMcp.owner==="ecc";const hasAihControl=model.catalog.mcp.some(function(control){return control.id===externalMcp.id});const unavailable=model.catalog.unavailableMcp.find(function(candidate){return candidate.id===externalMcp.id});const launch=externalMcp.url||([externalMcp.command].concat(externalMcp.args||[]).filter(Boolean).join(" "))||"manual configuration";const credentials=externalMcp.credentialRequirement.variables.length?externalMcp.credentialRequirement.variables.join(", "):"none declared";const provenance=model.catalog.eccMcpProvenance;const audit=auditDetails('<div class="cap">Provenance</div><div class="kv">'+kv("Repository",esc(provenance.repository))+kv("Pinned commit",scrollCode(provenance.commit))+kv("Source definition",esc(provenance.path))+kv("Content SHA-256",scrollCode(model.catalog.eccMcpApproval.sourceContentSha256))+(unavailable?kv("Configured runtime identity",scrollCode(unavailable.configuredIdentity)):"")+kv("Credential variables",esc(credentials))+kv("Supply",esc(externalMcp.supply))+kv("Addability",esc(externalMcp.addability))+'</div><p class="note">This source-locked declaration is not installed, contacted, scanned, attested, or represented as reachable.'+(approval?"":hasAihControl?" A separate AIH control row exists for this id.":unavailable?" "+esc(unavailable.reason):" This Core build has no AIH policy control for this id.")+'</p>');host.innerHTML='<div class="dhead"><h2>'+esc(item.id)+'</h2>'+close+'</div><div class="badges"><span class="b ext">ECC source metadata</span><span class="b">MCP</span></div><div class="cap">Component overview</div><p class="note">'+esc(externalMcp.description)+'</p><div class="kv">'+kv("Allowed tools / scope",esc(externalMcp.description))+kv("Transport",esc(externalMcp.transport))+kv("Declared launch",scrollCode(launch))+'</div><p class="note"><b>Usage context:</b> '+esc("Declared for "+externalMcp.transport+" use through "+launch+".")+'</p>'+componentStatus(false,approval?"An administrator must review and author approval for this exact pinned declaration.":unavailable?unavailable.reason:"This availability entry has no separate scanner verdict and no supported policy action.")+audit+(approval?'<div class="brow"><button type="button" class="btn sm" data-ecc-mcp-approval="'+esc(externalMcp.id)+'">Open approval authoring</button></div>':"");return}
...
const browserArgumentErrors=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const sources=[];const catalog=governanceValue.catalog||{};["reviewed","custom"].forEach(function(collection){(catalog[collection]||[]).forEach(function(candidate,index){sources.push({source:candidate&&candidate.source,path:"policy.governance.catalog."+collection+"["+index+"]"})})});const approvals=governanceValue.authority&&governanceValue.authority.approvals||[];approvals.forEach(function(approval,index){sources.push({source:approval&&approval.source,path:"policy.governance.authority.approvals["+index+"]"})});sources.forEach(function(entry){const source=entry.source;if(source&&(source.type==="package"||source.type==="stdio")&&!safeBrowserHttpsOrigin(source.registry)){errors.push(validationError(entry.path+".source.registry","must be an exact HTTPS origin"))}if(source&&source.type==="command"&&Array.isArray(source.args)){source.args.forEach(function(argument,index){if(!safeBrowserArgument(argument)){errors.push(validationError(entry.path+".source.args["+index+"]","must be a safe relative argument or exact HTTPS registry/index origin"))}})}});return errors};
  ...
  const browserSemanticErrors=function(policy){return policySemantics(policy).filter(function(error){return !/\.source\.args\[\d+\]: must be a safe relative argument/.test(error)}).concat(managedMcpPolicySemantics(policy),selectionDependencySemantics(policy),selectionAttributionSemantics(policy),kiroTransportSemantics(policy),browserArgumentErrors(policy),browserRootErrors(policy))};
