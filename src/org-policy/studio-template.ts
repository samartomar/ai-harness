...
const selectFrameworkAssetClosure=function(g,framework,asset){let added=0,blocked=null;const pending=[{id:asset.id,includeMembers:true}],expanded=new Set();while(pending.length&&!blocked){const next=pending.shift();const expansionKey=next.id+"|"+(next.includeMembers?"members":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);if(frameworkAssetExcluded(framework.id,next.id)){blocked=next.id;break}const target=frameworkAsset(framework.id,next.id);if(!target){blocked=next.id;break}if(!isFrameworkSelected(framework.id,next.id)){if(!selectFrameworkAsset(g,target.framework,target.asset)){blocked=next.id;break}added++}assetRequirementIds(target.asset,next.includeMembers).forEach(function(required){pending.push({id:required,includeMembers:false})})}return {added:added,blocked:blocked}};
...
const toggleFrameworkSelection=function(key,origin){
  ...
  if(group&&group.items.some(function(item){return item.id===found.asset.id})){
    const roots=frameworkSelectionRoots(group),requiredByRoots=roots.filter(function(root){return frameworkRequirementClosure(found.framework,[root]).has(found.asset.id)});
    if(origin==="center"){
...
byId("import-policy").addEventListener("click",function(){byId("policy-file").click()});byId("policy-file").addEventListener("change",function(event){readFile(event.target,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const problems=schemaErrors(model.schema,value,"").concat(policySemantics(value),selectionDependencySemantics(value),kiroTransportSemantics(value),policyTextSemantics(value));if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.policy=value;state.compositionClaims={};state.frameworkExclusions={};activePreset="custom";announce("Policy imported without transformation after schema and policy-grammar validation.");render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}})});
...
const browserArgumentErrors=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const sources=[];const catalog=governanceValue.catalog||{};["reviewed","custom"].forEach(function(collection){(catalog[collection]||[]).forEach(function(candidate,index){sources.push({source:candidate&&candidate.source,path:"policy.governance.catalog."+collection+"["+index+"]"})})});const approvals=governanceValue.authority&&governanceValue.authority.approvals||[];approvals.forEach(function(approval,index){sources.push({source:approval&&approval.source,path:"policy.governance.authority.approvals["+index+"]"})});sources.forEach(function(entry){const source=entry.source;if(source&&(source.type==="package"||source.type==="stdio")&&!safeBrowserHttpsOrigin(source.registry)){errors.push(validationError(entry.path+".source.registry","must be an exact HTTPS origin"))}if(source&&source.type==="command"&&Array.isArray(source.args)){source.args.forEach(function(argument,index){if(!safeBrowserArgument(argument)){errors.push(validationError(entry.path+".source.args["+index+"]","must be a safe relative argument or exact HTTPS registry/index origin"))}})}});return errors};
  ...
  const browserSemanticErrors=function(policy){return policySemantics(policy).filter(function(error){return !/\.source\.args\[\d+\]: must be a safe relative argument/.test(error)}).concat(selectionDependencySemantics(policy),kiroTransportSemantics(policy),browserArgumentErrors(policy),browserRootErrors(policy))};
