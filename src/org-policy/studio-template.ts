...
byId("import-policy").addEventListener("click",function(){byId("policy-file").click()});byId("policy-file").addEventListener("change",function(event){readFile(event.target,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const problems=schemaErrors(model.schema,value,"").concat(policySemantics(value),kiroTransportSemantics(value),policyTextSemantics(value));if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.policy=value;announce("Policy imported without transformation after schema and policy-grammar validation.");render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}})});
...
byId("import-decision").addEventListener("click",function(){byId("decision-file").click()});byId("decision-file").addEventListener("change",function(event){readFile(event.target,function(text){try{const value=JSON.parse(text);const problems=decisionProblems(value);if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.decision=structuredClone(value);announce("Decision imported for inspection only: unverified and not effective.");renderDecision()}catch(error){announce("Decision import rejected: "+(error&&error.message?error.message:"strict decision JSON required"),true)}})});
