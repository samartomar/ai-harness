...
const eccMcpInventoryRow=function(item){const approval=item.owner==="ecc";const hasAihControl=model.catalog.mcp.some(function(control){return control.id===item.id});const aihAvailabilityOnly=item.owner==="aih"&&!hasAihControl;const detail=item.description+" Transport "+item.transport+", supply "+item.supply+", addability "+item.addability+". This row does not install, contact, scan, attest, or claim reachability."+(aihAvailabilityOnly?" This hosted identity is runtime availability only and is not policy-projectable by the managed stdio projector.":"");return row("ECC MCP: "+item.id,detail,approval?"Approval authoring available — not installed":hasAihControl?"Available — represented by the shared AIH control":"Available — not policy-projectable",approval?"approval":"availability",approval?'<button type="button" class="btn sm" data-ecc-mcp-approval="'+esc(item.id)+'">Open approval</button>':"",approval?"ECC declares this at its pinned source; AIH records only a separately authored approval.":aihAvailabilityOnly?"AIH runtime and ECC declare this hosted identity, but the current policy projector cannot own it; it remains visible and unselectable.":"ECC assigns this entry to AIH. This source view does not create an AIH control.",item.id,undefined,' data-ecc-mcp-availability="'+esc(item.id)+'" data-ecc-mcp-provenance="only"',approval?"":"Runtime availability only")};
