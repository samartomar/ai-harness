...
const state={policy:structuredClone(model.initialPolicy),receipt:null,decision:null,compositionClaims:{}};
...
const policyProblems=function(){return schemaErrors(model.schema,state.policy,"").concat(policySemantics(state.policy),selectionDependencySemantics(state.policy),kiroTransportSemantics(state.policy),policyTextSemantics(state.policy))};
...
const selectFrameworkAssetClosure=function(g,framework,asset){let added=0;let blocked=null;const pending=[{id:asset.id,includeMembers:true}],expanded=new Set();while(pending.length&&!blocked){const next=pending.shift();const expansionKey=next.id+"|"+(next.includeMembers?"members":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);const target=frameworkAsset(framework.id,next.id);if(!target){blocked=next.id;break}if(!isFrameworkSelected(framework.id,next.id)){if(!selectFrameworkAsset(g,target.framework,target.asset)){blocked=next.id;break}added++}assetRequirementIds(target.asset,next.includeMembers).forEach(function(required){pending.push({id:required,includeMembers:false})})}return {added:added,blocked:blocked}};
...
const toggleFrameworkSelection=function(key,origin){const parts=String(key).split("|");const found=frameworkAsset(parts[0],parts[2]);if(!found){return}const previous=structuredClone(state.policy);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const group=g.externalSelections.find(function(item){return item.framework===found.framework.id});if(group&&group.items.some(function(item){return item.id===found.asset.id})){const roots=frameworkSelectionRoots(group);const requiredByRoots=roots.filter(function(root){return frameworkRequirementClosure(found.framework,[root]).has(found.asset.id)});if(origin==="center"){const wasEnterpriseComplete=manualCompositionChange();const removedByRoots=requiredByRoots.length?removeFrameworkSelectionRoots(g,found.framework,requiredByRoots):0;const removedDependents=removeFrameworkSelectionDependents(g,found.framework,found.asset.id);const rootsNamed=requiredByRoots.length?" ("+requiredByRoots.join(", ")+")":"";commitPolicy(previous,"Center deselected "+found.asset.id+"; removed "+requiredByRoots.length+" dependent root(s)"+rootsNamed+" and "+(removedByRoots+removedDependents)+" root or required component(s) from the exported policy."+compositionBreakNotice(wasEnterpriseComplete));return}if(roots.indexOf(found.asset.id)===-1){const unattributed=Array.isArray(group.unattributedItems)&&group.unattributedItems.indexOf(found.asset.id)!==-1;if(unattributed){announce(found.asset.id+" came from an imported rootless policy, so its direct selection provenance is unknown. Use the center inventory to remove it and every item that requires it.",true);render();return}announce(found.asset.id+" cannot be deselected from the left composition helper while "+(requiredByRoots[0]||"another explicit root")+" requires it. Use the center inventory to override the composed selection.",true);render();return}const remainingRoots=roots.filter(function(root){return root!==found.asset.id});const retainedByRoot=remainingRoots.find(function(root){return frameworkRequirementClosure(found.framework,[root]).has(found.asset.id)});const wasEnterpriseComplete=manualCompositionChange();const removed=removeFrameworkSelectionRoots(g,found.framework,[found.asset.id]);const related=found.asset.dependencies&&found.asset.dependencies.length||found.asset.members&&found.asset.members.length;const relation=related?" dependency-closed component(s)":" selection and unshared rider(s)";const message=retainedByRoot?"Removed the explicit root "+found.asset.id+", but the component remains selected because "+retainedByRoot+" requires it. Use the center inventory to remove the shared component and every dependent root.":"Deselected "+found.asset.id+"; removed "+removed+relation+" from the exported policy."+compositionBreakNotice(wasEnterpriseComplete);commitPolicy(previous,message);return}const conflict=selectionConflict(g,found.framework.id);
  ...
  const previousPreset=activePreset;const wasEnterpriseComplete=manualCompositionChange();const selected=selectFrameworkAssetClosure(g,found.framework,found.asset);if(selected.blocked){state.policy=previous;activePreset=previousPreset;announce("Selection blocked: required component "+selected.blocked+" cannot be authored alongside its existing curation or framework state; nothing changed.",true);render();return}markFrameworkSelectionRoots(found.framework.id,[found.asset.id]);const requirements=frameworkRequirementClosure(found.framework,[found.asset.id]);requirements.delete(found.asset.id);const related=found.asset.dependencies&&found.asset.dependencies.length||found.asset.members&&found.asset.members.length;const relation=related?" required component(s)":" declared rider(s)";commitPolicy(previous,"Selected "+found.asset.id+" with "+requirements.size+relation+": requested intent recorded with pinned sources."+compositionBreakNotice(wasEnterpriseComplete))};
...
document.addEventListener("click",function(event){if(!event.target.closest||!event.target.closest("#clear-policy")){return}
  state.policy=structuredClone(model.initialPolicy);state.editing=null;state.compositionClaims={};activePreset="custom";
...
const browserArgumentErrors=function(policy){const errors=[];const governanceValue=policy&&policy.governance;if(!governanceValue||typeof governanceValue!=="object"){return errors}const sources=[];const catalog=governanceValue.catalog||{};["reviewed","custom"].forEach(function(collection){(catalog[collection]||[]).forEach(function(candidate,index){sources.push({source:candidate&&candidate.source,path:"policy.governance.catalog."+collection+"["+index+"]"})})});const approvals=governanceValue.authority&&governanceValue.authority.approvals||[];approvals.forEach(function(approval,index){sources.push({source:approval&&approval.source,path:"policy.governance.authority.approvals["+index+"]"})});sources.forEach(function(entry){const source=entry.source;if(source&&(source.type==="package"||source.type==="stdio")&&!safeBrowserHttpsOrigin(source.registry)){errors.push(validationError(entry.path+".source.registry","must be an exact HTTPS origin"))}if(source&&source.type==="command"&&Array.isArray(source.args)){source.args.forEach(function(argument,index){if(!safeBrowserArgument(argument)){errors.push(validationError(entry.path+".source.args["+index+"]","must be a safe relative argument or exact HTTPS registry/index origin"))}})}});return errors};
  ...
  byId("policy-file").addEventListener("change",function(event){event.stopImmediatePropagation();readInput(event.currentTarget,function(text){try{const value=JSON.parse(text);if(!value||typeof value!=="object"||Array.isArray(value)){throw new Error("not an object")}const problems=schemaErrors(model.schema,value,"").concat(browserSemanticErrors(value),policyTextSemantics(value));if(problems.length){throw new Error(problems.slice(0,3).join("; "))}state.policy=value;state.editing=null;activePreset="custom";announce("Policy imported without transformation after schema and policy-grammar validation.");render()}catch(error){announce("Policy import rejected: "+(error&&error.message?error.message:"valid policy JSON required"),true)}},true);
  },true);
