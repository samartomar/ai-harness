...
const attributeRootlessSelection=function(group,framework){if(Array.isArray(group.roots)){return false}const selected=new Set(group.items.map(function(item){return item.id})),related=new Set();group.items.forEach(function(item){const asset=framework.assets.find(function(candidate){return candidate.id===item.id});if(!asset){return}assetRequirementIds(asset,true).forEach(function(id){if(selected.has(id)){related.add(id)}})});let roots=group.items.map(function(item){return item.id}).filter(function(id){return !related.has(id)});if(!roots.length){roots=group.items.map(function(item){return item.id})}const reachable=frameworkRequirementClosure(framework,roots),unattributed=group.items.map(function(item){return item.id}).filter(function(id){return !reachable.has(id)});group.roots=roots;if(unattributed.length){group.unattributedItems=unattributed}else{delete group.unattributedItems}return true};
