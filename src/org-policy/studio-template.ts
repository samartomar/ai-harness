...
const toggleFrameworkSelection=function(key,origin){
  ...
  if(group&&group.items.some(function(item){return item.id===found.asset.id})){
    ...
    if(origin==="center"){
      ...
      if(!commitPolicy(previous,"Center deselected "+found.asset.id+"; removed "+requiredByRoots.length+" dependent root(s)"+rootsNamed+" and "+(removedByRoots+removedDependents)+" root or required component(s) from the exported policy. Left composition helpers will not restore it until you re-select it in the center."+compositionBreakNotice(wasEnterpriseComplete))){state.frameworkExclusions=previousExclusions;render()}
      return
    }
