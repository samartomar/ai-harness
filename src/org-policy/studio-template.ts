...
const toggleFrameworkSelection=function(key){const parts=String(key).split("|");const found=frameworkAsset(parts[0],parts[2]);if(!found){return}const previous=structuredClone(state.policy);const g=ensureGovernance();if(!Array.isArray(g.externalSelections)){g.externalSelections=[]}const group=g.externalSelections.find(function(item){return item.framework===found.framework.id});if(group&&group.items.some(function(item){return item.id===found.asset.id})){const roots=frameworkSelectionRoots(group);if(roots.indexOf(found.asset.id)===-1){const unattributed=Array.isArray(group.unattributedItems)&&group.unattributedItems.indexOf(found.asset.id)!==-1;if(unattributed){announce(found.asset.id+" came from an imported rootless policy, so its direct selection provenance is unknown. Clear the imported selection or replace it with explicit roots before removing it individually.",true);render();return}const requiredByRoot=roots.find(function(root){return frameworkRequirementClosure(found.framework,[root]).has(found.asset.id)});announce(found.asset.id+" cannot be deselected while "+(requiredByRoot||"another explicit root")+" requires it. Deselect the dependent root first.",true);render();return}const remainingRoots=roots.filter(function(root){return root!==found.asset.id});const retainedByRoot=remainingRoots.find(function(root){return frameworkRequirementClosure(found.framework,[root]).has(found.asset.id)});const wasEnterpriseComplete=manualCompositionChange();const removed=removeFrameworkSelectionRoots(g,found.framework,[found.asset.id]);const related=found.asset.dependencies&&found.asset.dependencies.length||found.asset.members&&found.asset.members.length;const relation=related?" dependency-closed component(s)":" selection and unshared rider(s)";const message=retainedByRoot?"Removed the explicit root "+found.asset.id+", but the component remains selected because "+retainedByRoot+" requires it. Deselect that dependent root to remove the shared component.":"Deselected "+found.asset.id+"; removed "+removed+relation+" from the exported policy."+compositionBreakNotice(wasEnterpriseComplete);commitPolicy(previous,message);return}const conflict=selectionConflict(g,found.framework.id);
