...
const preparePolicyImport=function(value,problemsFor){const migration=legacyWorkbenchMcpMigration(value),imported=migration?migration.policy:value,problems=problemsFor(imported);if(problems.length){throw new Error(problems.slice(0,3).join("; "))}return {policy:imported,message:migration?migration.message:"Policy imported without transformation after schema and policy-grammar validation."}};
...
const inferFrameworkExclusions=function(policy,structuralRoots){const result={},governanceValue=policy&&policy.governance,selections=governanceValue&&Array.isArray(governanceValue.externalSelections)?governanceValue.externalSelections:[];selections.forEach(function(group){if(!Array.isArray(group.roots)){return}const framework=model.catalog.frameworks.find(function(candidate){return candidate.id===group.framework});if(!framework){return}const selected=new Set((group.items||[]).map(function(item){return item.id})),structural=new Set(structuralRoots&&structuralRoots[group.framework]||[]),excluded=new Set(),expanded=new Set(),pending=group.roots.map(function(id){return {id:id,includeSuggestions:!structural.has(id)}});while(pending.length){const next=pending.shift(),expansionKey=next.id+"|"+(next.includeSuggestions?"suggestions":"structural");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);const asset=framework.assets.find(function(candidate){return candidate.id===next.id});if(!asset){continue}assetMandatoryRequirementIds(asset).forEach(function(required){if(selected.has(required)){pending.push({id:required,includeSuggestions:false})}});const suggested=next.includeSuggestions?[].concat(asset.members||[],asset.riders||[]):[];suggested.forEach(function(id){if(!selected.has(id)){excluded.add(id);return}pending.push({id:id,includeSuggestions:false})})}if(excluded.size){result[group.framework]=Array.from(excluded)}});return result};
...
const selectFrameworkAssetClosure=function(g,framework,asset){let added=0,blocked=null;const pending=[{id:asset.id,includeMembers:true,required:true}],expanded=new Set();while(pending.length&&!blocked){const next=pending.shift();const expansionKey=next.id+"|"+(next.includeMembers?"members":"structural")+"|"+(next.required?"required":"suggested");if(expanded.has(expansionKey)){continue}expanded.add(expansionKey);if(frameworkAssetExcluded(framework.id,next.id)){if(next.required){blocked=next.id}continue}const target=frameworkAsset(framework.id,next.id);if(!target){if(next.required){blocked=next.id}continue}if(!isFrameworkSelected(framework.id,next.id)){if(!selectFrameworkAsset(g,target.framework,target.asset)){if(next.required){blocked=next.id}continue}added++}assetMandatoryRequirementIds(target.asset).forEach(function(required){pending.push({id:required,includeMembers:false,required:true})});if(next.includeMembers){(target.asset.members||[]).forEach(function(suggested){pending.push({id:suggested,includeMembers:false,required:false})})}(target.asset.riders||[]).forEach(function(suggested){pending.push({id:suggested,includeMembers:false,required:false})})}return {added:added,blocked:blocked}};
...
hostSelectAll.addEventListener("click",function(){var ids=(model.catalog.hosts||[]).map(function(host){return host.id});var previous=structuredClone(state.policy);ensureGovernance().supportedClis=ids;commitPolicy(previous,"All "+ids.length+" registry CLIs explicitly sanctioned. Unsanctioned selected or detected CLIs are refused by the engine.")});
