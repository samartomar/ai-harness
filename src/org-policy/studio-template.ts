...
const render=function(){byId("posture").value=state.policy.minimumPosture||"vibe";syncFrameworkSelect();renderRows();renderEccHookControls();renderEccMcpApprovals();renderComposition();renderReceipt();renderPreview();syncRail();paintHosts();paintShell();byId("dispositionable-findings").textContent=model.findings.dispositionable.join(" | ");byId("hard-blockers").textContent=model.findings.fenced.join(" | ");if(typeof window.__aihPolicyWorkbenchEnhanceRows==="function"){window.__aihPolicyWorkbenchEnhanceRows()}};
