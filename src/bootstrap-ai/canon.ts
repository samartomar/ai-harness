...
export function sharedCanonicalBlockBody(dir: string): string {
  return lines(
    "## Start here",
    "",
    "This is an intentional always-loaded summary for bootloaders. The full source",
    `of behavior is \`${dir}/rules/agent-behavior-core.md\`; repo-specific rules`,
    `belong in \`${dir}/architecture.md\`, \`${dir}/conventions.md\`, or`,
    `\`${dir}/project-guardrails.md\`, not in this shared adapter block.`,
    "",
...
export function agentBehaviorCoreDoc(dir: string): string {
  return lines(
    ...
    "",
    "These invariants are intentionally repeated in the bootloader shared block so",
    "every tool receives them even when it only auto-loads one root file. Add",
    `repo-specific rules to \`${dir}/architecture.md\`, \`${dir}/conventions.md\`,`,
    `or \`${dir}/project-guardrails.md\` and prune duplicates there.`,
    "",
...
export function ruleRouterDoc(
  ...
  return lines(
    ...
    "before other work — it also covers enhancing `project-guardrails.md` and",
    "pruning duplicate rules back to their owning canon file.",
    "",
    "## Task routing",
    "",
    "### Implementation",
...
export function adapterNote(cli: Cli, dir: string): string {
  ...
  return lines(
    ...
    "source of repo truth. They own loading facts only; do not add project behavior",
    `rules here. Put those in \`${dir}/architecture.md\`, \`${dir}/conventions.md\`,`,
    `or \`${dir}/project-guardrails.md\`.`,
    "",
    "## Entry points",
    "",
    `- ${m.entry}`,
...
export function regenerationDoc(dir: string, bootloaders: string[]): string {
  return lines(
    ...
    "- Do not add repo-specific behavior rules to adapters or the shared block; put",
    `  them in \`${dir}/architecture.md\`, \`${dir}/conventions.md\`, or`,
    `  \`${dir}/project-guardrails.md\`, then prune duplicate copies.`,
...
export function harnessUpdateDoc(dir: string): string {
  return lines(
    ...
    "- When a standing rule appears in the wrong file, move it to its owner and",
    "  leave only a pointer; do not keep duplicate canon in sync by hand.",
    "",
    "## To update",
    "",
...
export function otherToolsDoc(dir: string): string {
  return lines(
    ...
    "`aih bootstrap-ai --cli <tool>` once aih adds native support for it. Do not",
    "store project behavior rules in unsupported-tool pointers; route them to the",
    "owning canon file instead.",
  );
}
