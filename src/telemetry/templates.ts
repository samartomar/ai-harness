...
export function fetchAnalyticsScript(): string {
  return lines(
    ...
    "// run at generation time and prints the curl equivalents unless given --run.",
    "// On --run it emits { usage_report, skills } — the shape `aih report --org` reads.",
    "",
    ...
    "}",
    "",
    "// The curl equivalents — printed so each request is auditable before it runs.",
    ...
    '  ].join(" \\\\\\n");',
    "}",
    "console.log(curlFor(usageUrl));",
    "console.log(curlFor(skillsUrl));",
    "",
