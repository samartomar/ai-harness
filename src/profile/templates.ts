 ...
 * builders (no dates, stable ordering) so golden-file tests stay stable.
 * `CLAUDE.md` is a THIN pointer (< 30 lines) into the canonical context dir; the
 * `.mdc` files carry stack-specific rules. Crucially, only facts/commands the
 * repo ACTUALLY has are emitted — no invented `vitest`/`lint` commands.
 */
...
/**
 * The thin `CLAUDE.md` adapter: a pointer, never the rulebook. Routes to
 * `contextDir`, surfaces ONLY the detected stack + real commands, stays < 30 lines.
 */
export function renderClaudeMd(stack: RepoStack, contextDir: string): string {
  return lines(
    ...
    `Canonical rules, architecture, and context live in \`${contextDir}/\` and \`.cursor/rules/\`.`,
    ...
    "## Guardrails",
    "",
    "- Run the test command above (if any) before claiming a change works.",
    `- Keep this file thin; durable guidance goes in \`${contextDir}/\`.`,
  );
}
