...
export function usageRecorderScript(): string {
  return lines(
    ...
    "    const name = str(ti.subagent_type, ti.subagentType, ti.agent_name, ti.agentName, ti.agent, ti.name, p.subagent_type, p.subagentType);",
    ...
    "  }",
    '  if (t === "Skill" || t === "skill") {',
    "    const name = str(ti.skill_name, ti.skillName, ti.skill?.name, ti.skill?.id, ti.command, ti.skill, ti.name, ti.id, p.skill_name, p.skillName, p.skill?.name, p.skill?.id);",
    '    if (!name) { ev.kind = "tool"; ev.name = t; return ev; }',
    '    ev.kind = "skill"; ev.name = name;',
    "    ev.source = sourceOf(ev.name, ti, p);",
    "    return ev;",
