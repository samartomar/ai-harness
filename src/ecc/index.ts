...
const CODEX_INSTALL_MERGE_SCRIPT = [
  ...
  '  const operations = governed ? selectedOperations.filter((operation) => classifyGovernedCodexOperation(operation) === "ecc-content") : selectedOperations;',
  '  if (governed) { const destinations = new Set(); for (const operation of operations) { const destination = normalize(operation.destinationPath); const collisionKey = destination.normalize("NFC").toLowerCase(); if (destinations.has(collisionKey)) throw new Error("normalized governed Codex ECC destination collision: " + destination); destinations.add(collisionKey); } }',
  ...
  '  const array = (value) => Array.isArray(value) && value.every((entry) => typeof entry === "string");',
  '  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate) || candidate.schemaVersion !== 1 || candidate.managedBy !== "aih" || candidate.agentsBlock !== true || !candidate.codexToml || typeof candidate.codexToml !== "object" || Array.isArray(candidate.codexToml) || !array(candidate.codexToml.rootKeys) || !array(candidate.codexToml.tables) || !array(candidate.codexToml.mcpServers) || !candidate.codexToml.tableKeys || typeof candidate.codexToml.tableKeys !== "object" || Array.isArray(candidate.codexToml.tableKeys) || !Object.values(candidate.codexToml.tableKeys).every(array)) throw new Error("malformed live Codex AIH state");',
  "  return candidate;",
  "}",
  ...
  '  return body.length === 4 && /^(?:\\[mcp_servers\\.chrome-devtools\\]|\\[mcp_servers\\."chrome-devtools"\\])$/.test(body[0]) && body[1] === "command = \\"npx\\"" && body[2] === "args = [\\"-y\\", \\"chrome-devtools-mcp@latest\\"]" && body[3] === "startup_timeout_sec = 30";',
  "}",
  ...
  "  let outside = parsed.lines; if (parsed.fence) outside = [...parsed.lines.slice(0, parsed.fence.begin), ...parsed.lines.slice(parsed.fence.end + 1)];",
  '  const chrome = serverTables(outside, "chrome-devtools"); const claimsChrome = claimed.has("chrome-devtools");',
  ...
  '  if (!parsed.fence && liveState && [...claimed].some((name) => name !== "chrome-devtools")) throw new Error("live AIH MCP state is orphaned from its managed fence");',
  "  const retained = parsed.fence ? parsed.fence.sections : []; const present = new Set(retained.map((section) => section.name));",
  "  for (const line of outside) { const match = line.match(/^\\[mcp_servers\\.(?:\"([^\"]+)\"|'([^']+)'|([A-Za-z0-9_-]+))\\]\\s*(?:#.*)?$/); if (match) present.add(match[1] || match[2] || match[3]); }",
  '  if (claimsChrome && chrome.length === 1) present.delete("chrome-devtools");',
  "  const installed = []; const additions = []; for (const [name, server] of Object.entries(mcpSpec.servers)) { if (present.has(name)) continue; additions.push({ name, text: renderScopedSection(name, server) }); installed.push(name); }",
  '  const sections = [...retained.map((section) => ({ name: section.name, text: section.lines.join("\\n").replace(/\\n+$/, "") })), ...additions];',
  '  const block = "# >>> aih managed (mcp) >>>\\n" + sections.map((section) => section.text).join("\\n\\n") + "\\n# <<< aih managed (mcp) <<<";',
  '  let mergedLines; if (parsed.fence) { const before = parsed.lines.slice(0, parsed.fence.begin); const legacy = serverTables(before, "chrome-devtools"); if (claimsChrome && legacy.length === 1) before.splice(legacy[0].begin, legacy[0].end - legacy[0].begin); mergedLines = [...before, block, ...parsed.lines.slice(parsed.fence.end + 1)]; } else { const before = parsed.lines.slice(); if (claimsChrome && chrome.length === 1) before.splice(chrome[0].begin, chrome[0].end - chrome[0].begin); mergedLines = before.join("\\n").replace(/\\n+$/, "").split("\\n"); if (mergedLines.length === 1 && mergedLines[0] === "") mergedLines = []; mergedLines.push(...(mergedLines.length > 0 ? ["", block] : [block])); }',
  '  let merged = mergedLines.join("\\n").replace(/^\\n+/, "").replace(/\\n+$/, "") + "\\n"; if (/\\r\\n/.test(existingConfig)) merged = merged.replace(/\\n/g, "\\r\\n");',
  "  return { existingConfig, merged, installed, retained: retained.map((section) => section.name) };",
  "}",
  'function installScopedMcps() { const plan = planScopedMcps(); if (!plan) return []; const current = readSafeOptional(configPath) || ""; if (current !== plan.existingConfig) { const refreshed = planScopedMcps(); if (!refreshed || (readSafeOptional(configPath) || "") !== refreshed.existingConfig) throw new Error("Codex MCP config changed during apply"); fs.writeFileSync(prepareDestination(configPath), refreshed.merged, "utf8"); state.codexToml.mcpServers = [...new Set([...(state.codexToml.mcpServers || []), ...refreshed.retained, ...refreshed.installed])].sort(); return refreshed.installed; } fs.writeFileSync(prepareDestination(configPath), plan.merged, "utf8"); state.codexToml.mcpServers = [...new Set([...(state.codexToml.mcpServers || []), ...plan.retained, ...plan.installed])].sort(); return plan.installed; }',
