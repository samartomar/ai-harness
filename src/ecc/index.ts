...
const CODEX_INSTALL_MERGE_SCRIPT = [
  ...
  "const trustedHome = fs.realpathSync(homeDir);",
  "function assertInsideHome(target) {",
  "  const absolute = path.resolve(target);",
  "  const relative = path.relative(trustedHome, absolute);",
  '  if (!relative || relative === ".." || relative.startsWith(".." + path.sep) || path.isAbsolute(relative)) throw new Error("refusing Codex destination outside trusted home: " + absolute);',
  "  return { absolute, parts: relative.split(path.sep) };",
  "}",
