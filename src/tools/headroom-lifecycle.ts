...
export async function activateHeadroom(
  ...
  try {
    ...
    const kept =
      ...
      previous.receipt.deactivation === undefined &&
...
export async function verifyActiveHeadroom(
  ...
  if (current.receipt.deactivation !== undefined) {
    return {
      state: "blocked",
      detail: `Headroom deactivation is incomplete: ${incompleteHostsDetail(current.receipt.deactivation.incompleteHosts)}`,
      changed: false,
    };
  }
  let changed = false;
...
function incompleteHostsDetail(hosts: readonly HeadroomIncompleteHost[]): string {
  const named = hosts.map(({ host, path, reason }) => `${host} (${path}): ${reason}`).join("; ");
  return `${named}. The Headroom runtime and activation receipt were kept. Delete that entry, restore AIH's generated bytes, or move it outside the aih-managed block, then rerun --deactivate-headroom (or re-activate with --activate-headroom --accept-headroom-egress)`;
}

/**
 * A deactivation that could not clean every recorded host keeps the runtime and
 * receipt, and records which hosts and why, so later runs neither re-register
 * Headroom nor report it active. Returns the detail to report.
 */
export function recordIncompleteHeadroomRemoval(
  ctx: PlanContext,
  incompleteHosts: readonly HeadroomIncompleteHost[],
): string {
  const layout = headroomLayout(ctx);
  const current = readHeadroomReceipt(layout);
  if (current.state === "valid" || current.state === "stale") {
    writeHeadroomReceipt(
      layout,
      { ...current.receipt, deactivation: { incompleteHosts: [...incompleteHosts] } },
      current.sha256,
    );
  }
  return `Headroom deactivation is incomplete: ${incompleteHostsDetail(incompleteHosts)}`;
}
