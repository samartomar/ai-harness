...
export function setupTasksDoc(dir: string, stack: RepoStack): string {
  ...
  return lines(
    ...
    "",
    "Do not open `.env*` or `secrets/**` while gathering evidence — `.env.example` /",
    "`.env.sample` are readable templates. Run `aih secrets --verify` and cite only",
    "the redacted finding/status.",
    "For large repos, do not map the whole tree by reading files. Run `aih doctor`",
    "first. If `large-repo graph safety` fails, warn once and continue with bounded",
    "reconnaissance (`rg`/`fd`) — code-review-graph is advisory, not a gate; repair",
    "it only when helper repair is the assigned task.",
    "",
...
export function validationDoc(dir: string, stack: RepoStack): string {
  ...
  return lines(
    ...
    "",
    "### 5. MCP wired",
    "- `.mcp.json` present; stdio servers need their configured runners (for example `uv`/`uvx` or `npx`).",
    "- ⚠️ GAP if a configured runner is missing → install it. 🔧 WORKAROUND: remove servers you can't launch.",
    "",
    "### 6. Dev tools",
    ...
    "",
    "### 10. Secrets",
    "- Do not open `.env*` or `secrets/**` (`.env.example` / `.env.sample` are readable templates). Validate with `aih secrets --verify` instead.",
    "- `aih secrets --verify` must pass before you claim no plaintext secrets are committed.",
    "- For Claude-targeted repos, `.claudeignore` + `.claude/settings.json` deny rules must be present.",
    "- For non-Claude targets, their absence is expected; validate secrets with `aih secrets --verify` plus gitleaks/pre-commit guardrails.",
