...
export function renderSkillGovernance(model: V9SkillGovernance): string {
  ...
  const approvalRow = approvals
    ? `<div class="row"><span class="k">approval verdicts</span><span class="v"${approvals.YELLOW > 0 || (approvals.RED ?? 0) > 0 || (approvals.UNKNOWN ?? 0) > 0 ? ' style="color:var(--warn)"' : ""}>GREEN ${approvals.GREEN} · YELLOW ${approvals.YELLOW}${approvals.RED === undefined ? "" : ` · RED ${approvals.RED} · UNKNOWN ${approvals.UNKNOWN ?? 0}`}</span></div>`
    : "";
