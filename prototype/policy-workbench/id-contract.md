...
| Still on the legacy shell after S7 | — | studio-bring-your-own-paths "separates organization-owned intake… left navigation" (`#byo-actions`, `#ecc-mcp-actions`, ECC MCP approval drawer, custom-hook note drawer); supported-cli-subsets "requires explicit managed MCP opt-in…" (reads `#open-ecc-mcp` in `#panel-author`); generate.test "has semantic controls…" (`main#workbench`, `section.group`); legacy template/string tests (studio-template-data, studio-vet-verdicts, studio-finding-model "stops claiming…", header-restyle, design-foundation, catalog-card-icon); legacy visual baselines (visual-shell "admin shell header and kind ledger…"); legacy-download-characterization (S0, by design) | n/a | legacy lane |
