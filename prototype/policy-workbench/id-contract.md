...
| Still on the legacy shell after S3 (need later screens) | — | S4: compact-setup "keeps the catalog anchored…", "keeps Back to catalog available…", "closes the mobile inspector…", scrolling.spec.ts "keeps the mobile Workbench inspector…"; S6: compact-setup "unifies references…", "opens setup explanations…", "keeps compact controls usable…", "keeps every tab dense…"; S7: generic-journeys "keeps requests and local draft bytes…", artifact.spec.ts (packed artifact has no new-shell fixture yet); owner decision: scrolling.spec.ts "uses the document for desktop and narrow Workbench scrolling" (the new shell scrolls `#wb-main`, not the document) | n/a | legacy lane |
