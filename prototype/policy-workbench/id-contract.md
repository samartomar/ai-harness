...
| Still on the legacy shell after S4/S5/scan (need later screens) | — | S6 (admin-org): compact-setup "keeps the catalog anchored…" (reads `#deployment-readiness`; its layout-width assertions also need re-pointing to the new frame), "unifies references…", "opens setup explanations…", "keeps compact controls usable…", "keeps every tab dense…"; S7: generic-journeys "keeps requests and local draft bytes…", artifact.spec.ts (packed artifact has no new-shell fixture yet) | n/a | legacy lane |
