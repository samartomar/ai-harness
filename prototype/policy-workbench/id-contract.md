...
| — | visual baselines `new-shell-org-{light,dark,375-light}`, `new-shell-acme-{light,dark,375-light}` (win32; screen opened from `#nav-rail [data-wb-nav]`, `#status` masked) | D (plan S9, org and additions) | n/a (new baselines) | browser/visual-shell.spec.ts "new shell screens" |
...
| `#announcement` error state: legacy `className = "announce error"` (the `.error` class) | `data-wb-tone="error"` (`"info"` otherwise) on the same `#announcement`; the message text and `#status` mirror are unchanged | review follow-up | COSMETIC (styling hook only; no test or runtime read the class) | new-shell-download-compat.test.ts (messages) |
| `#download` accessible name: legacy `aria-label="Download"` / `title="Download"` | `aria-label="Publish (download the policy file)"`, `title="Publish: download the policy file"`; same id, same click behaviour and bytes | review follow-up | COSMETIC (label only; every spec locates it by `#download`) | new-shell-download-compat.test.ts (goldens byte-equal) |
| `[data-workbench-source-rail]` (with `[data-workbench-source-tab]`, `#workbench-source-rail-title`) inside `[data-workbench-catalog-layout]`, left of the register | same element, same hooks, moved to the nav rail's "Catalog scopes" section `#nav-rail [data-wb-catalog-scopes]` (admin-sources.html); `mountWorkbench` option `mountSourceRail`, shell `mountCatalogScopes`; a scope click opens the sources screen from any screen. The mobile `#workbench-source-filter` picker stays in the sources masthead | fidelity port | compact-setup "keeps the catalog anchored…": locator unchanged; geometry re-pointed from "rail inside the layout" to "rail right edge ≤ `#wb-main` left edge", plus `#nav-rail [data-workbench-source-rail]` count 1 (register/inspector ordering and widths unchanged). scrolling "uses the main panel…": the source rail no longer sits over `#wb-main`, so its wheel-moves-main check becomes "a wheel over the rail scrolls neither the rail nor the document" (the catalog wheel check on `#wb-main` is unchanged) | browser/compact-setup.spec.ts; browser/scrolling.spec.ts; new-shell-sources.test.ts (adds the nav-rail placement and the scope-opens-sources checks) |
| `.workbench-draft-counts` in the draft summary; "Build your policy" heading and intro; `[data-workbench-exposure-open]`, `.workbench-draft-review > button[data-workbench-draft-open]` | counts (same text, `Controls <strong>n</strong>`) in the sources masthead's title line where the prototype has "Source policy"; heading and intro `sr-only`; the two buttons as the filter bar's right-hand pills; all still inside `#framework-rows` | fidelity port | y (textContent `Controls 0Selections 0Requests 1` unchanged) | studio-fulfillment-affordance; studio-aih-mcp-requests; generate "Controls 0"; generic-journeys; compact-setup |
| card `Access: …` line and `[data-workbench-row-detail]` status line (visible text) | same elements and text, `sr-only` (the prototype card has no such lines; the inspector shows both); `[data-workbench-row-evidence]` is the footer status pill (tone via `data-workbench-evidence-tone`); `.workbench-row-expand` is an icon button (aria-label unchanged) | fidelity port | y (every assertion reads textContent or aria names) | studio-fulfillment-affordance; generic-journeys; new-shell-sources |
| — | `[data-wb-review-count]` (header Review Changes badge: draft entry count, hidden at 0), `[data-wb-catalog-scopes]` | fidelity port | n/a (new hook) | new-shell-sources.test.ts |
| visual baselines `new-shell-frame-{light,dark,375-light}`, `new-shell-org-*`, `new-shell-acme-*` | regenerated deliberately for the faithful frame and sources port; the org and additions baselines change only through the shared frame (header, nav rail, ledger, inspector rail) and the `.btn` control style | fidelity port | pixels n (cosmetic) | browser/visual-shell.spec.ts |
