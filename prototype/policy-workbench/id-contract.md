...
| Still on the legacy shell after S6 | — | S7: generic-journeys "keeps requests and local draft bytes…", artifact.spec.ts (no new-shell packed fixture yet), generate.test.ts "keeps curation and preserved evidence detail…", journeys.spec.ts (curation editor, protected form, `data-view-tab`) | n/a | legacy lane |
