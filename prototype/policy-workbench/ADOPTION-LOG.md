...
## Fidelity port — changes screen (admin-changes.html)

- The prototype draws this in the inspector rail beside the catalog; the product keeps it as the "Policies & Publish" screen in `#wb-main`, now full-bleed (like sources) with admin-changes.html's rail markup: head (title, change-count chip, selection counts, Copy JSON / Review draft / Policy exposure, Changes / Whole file), "Staged policy delta" with the filename from `#policy-download-name`, the editor box (line numbers, green added / amber removed rows, keys in primary and strings in secondary as text spans), the note, and the Publish footer ("Publish downloads <file>. Move it into your policy folder and commit it.").
- Not built (no product data or behaviour): Reset to template, the "Ready to publish" pill, "86 wait for review", the toolbar's separate Download (the footer Publish is the one download action on the screen). Whole file (the default the tests pin) stays the readonly `#config-preview` and `#report-preview` textareas, styled as the editor.
- Screenshots: `screenshots/fidelity-changes-{proto,product}-{1440-dark,1440-light,375}.png` (product: one MCP added, Changes view).
- Functional walk (Playwright, `aih-policy-workbench.html`), 25 checks, all passed: file menu open / Escape / focus; import policy valid and invalid; import evidence and decision (malformed, reported, draft unchanged); Check Policy; header Publish download equals `#config-preview`; changes screen defaults to Whole file with "No changes from the starting policy"; after Add to draft the header Review Changes opens it with "78 changed lines"; Changes shows added rows with painted keys and hides `#json-editor`; Whole file brings `#config-preview` back; Copy JSON writes the policy to the clipboard (announcement "Policy JSON copied to the clipboard."); Review draft and Policy exposure open those inspector views; the footer shows the filename typed in the file menu; the footer Publish keeps the header's gate ("Download blocked: enable managed MCP projection") and, after enabling projection on Organization, downloads `team-policy.json` equal to the draft; 375 has no horizontal scroll and the footer Publish is visible; zero page errors, CSP violations and network requests.

## Fidelity port — scan review screen (admin-scan.html)

- Full-bleed like sources: masthead "Scan Review — n need review" with the catalog's items / reports included / currently verified; column 1 "1 · The scan at a glance": Passed / Review / Not scanned tiles from the prepared catalog's report totals (`sourceEvidenceSummary` summed over sources, via the new `scanGlance` option; they sum to the ledger's catalog size), "What each result means" (the product's own definitions of the three tiles), "Do I need to run a scan?" (scans run in a target repository; Import evidence / Import decision), "When a source updates" (a report binds one revision and digest); column 2 "2 · Needs review, grouped": the finding model as the prototype's group card ("Findings an administrator decides", 8 kinds) and dashed card ("Nobody can decide these", 6 hard blockers), the finding-model disclosure, then the imported receipt ("Approval / evidence") and decision cards. Column 3, the short item report, is the shared inspector rail (item port).
- Not built (no product data or behaviour): Group by (Meaning / Source / Kind), the "4 scanners · version · pin" line, per-group Keep held back / Review one by one and example lines, the per-finding decision radios and reason field (decisions are imported, never authored here).
- Screenshots: `screenshots/fidelity-scan-{proto,product}-{1440-dark,1440-light,375}.png`.
- Functional walk (Playwright, `aih-policy-workbench.html`), 21 checks, all passed: the common header walk (file menu, imports valid / invalid / malformed, Check Policy, Publish download); masthead "Scan Review — 3 need review"; tiles sum to the catalog count; Import evidence and Import decision open the file choosers for `#evidence-file` / `#decision-file`; an evidence receipt import lists `approval-walk` as escaped preflight rows with the draft unchanged and Preserve approval subjects disabled; receipt details open; a schema-valid decision import fills `#decision-rows` and `#decision-export` (draft unchanged); Download decision saves `aih-governance-decision.json`; the finding-model disclosure opens with `#hard-blockers` visible; 375 has no horizontal scroll; zero page errors, CSP violations and network requests.
## Fidelity port — project page (user-trim.html, user-start.html, user-shell.js)

- Method as `88b76a06` / `deac13d2`: the prototype's markup and Tailwind classes rebuilt in `user-door.ts` (literal dark hexes as the `wb-*` theme tokens), every model string through `textContent`. The page is user-trim.html's frame: `h-11` header (aih Policy · User, Save, theme toggle), the provenance strip (policy file, source kind, valid, verified; saves to `aih-project-policy.json`), the Trimming strip with For (type select) and the project-name field, the sources rail (kept / offered per source and per kind, dot shows the kind's common choice), the six-tile kind ledger (five kinds plus All items; no token tile, D6), the page head with Set all N, item cards (kind glyph and chip, asset id, origin and source chips, Required / Optional / Skip segmented radios; changed cards take the primary border, skipped ones fade and strike through), and "What your AI carries" (changes chip, For / On / Required / Optional / Skipped, Set up for, Save and use with the two files, Reset and Save footer).
- Invalid and unbound source: user-start.html's dashed "No organization policy found" block — "The organization policy could not be read" with `#user-source-error`, or the missing server data, plus where the policy comes from; the rail, ledger and summary are hidden, both Saves disabled.
- Kept hooks: `#user-door`, `#theme-toggle`, `#user-policy-source[data-valid]`, `#user-trim-list > li[data-asset-id][data-use]` with a `radiogroup` of `radio`s (arrow keys, roving tabindex), `#user-for-name` (required, aria-required), `#user-for-type`, `#user-ai-tools` / `#user-ai-tool-<cli>`, `#user-save` (aria-describedby `user-save-message`), `#user-save-message`, `#user-trim-counts`, `#user-source-error`, `#user-trim-empty`, `#user-trim-ambiguous`. New: `#user-reset`, `[data-user-set-all]`, `[data-user-header-save]`, `[data-user-changes]`, `[data-user-ledger-tile]`, `html[data-wb-door="user"]` (theme glyph CSS, 4 additive lines in wb-tokens.css).
- Not built (no product data): the start page's templates and "Use as it is" (no starting-point choice in the product; the product page is the trim page, and the start pair compares the invalid state with the start page's no-policy case), categories, token costs and the Token Budget tile (D6), overlap / needs warnings, last-changes log, Vibe / Enterprise, org switcher and AI-tools header popover (AI tools stay in the right panel), Admin page link, Check Selection, avatar, panel toggles, the `@aihq/*` versions, "aih init . --apply" (the product text says only to commit the file).
- Screenshots (1440×900 dark, light; 375 light): `screenshots/fidelity-user-trim-{proto,product}-*` (user-trim.html vs `user-door.html`), `fidelity-user-start-{proto,product}-*` (user-start.html `?source=none` vs `user-door-invalid.html`). The prototype is not responsive at 375; the product stacks strip, ledger (3 columns), cards and the panel, rail hidden, no horizontal scroll.
- Functional walk (Playwright, strict offline CSP), 48 checks, all passed: every choice on all five cards (Required, Skip, Optional; `data-use` and `aria-checked`); arrow keys Right/Down/Left/Up with wrap and focus following, one tabbable radio per group; counts, changes chip and All items tile update live; Set all Skip / Required; Reset; empty and blank name and no AI tool refused with "Not saved: …" in `#user-save-message`; theme toggle and its label; the header Save downloads `aih-project-policy.json`; invalid page: `data-valid="false"`, `<marker>` shown as text, both Saves disabled, no rows; the S0 golden model rendered as a user page (ecc/pack Required, mcp/github Skip, " Payments API ", claude + codex) downloads 564 bytes identical to `tests/org-policy/workbench/goldens/aih-project-policy.json`; at 375 no horizontal scroll, a choice works, Save reachable; zero page errors, zero CSP violations, zero network requests on every page.
## Fidelity port — organization screen (admin-org.html)

- Method as `88b76a06` / `deac13d2`. `shell/screen-frame.ts` (new): the prototype's per-screen frame — mono kicker over the 22 px mono title in a subhead band, the centre on `bg-wb-cards`, and the screen's own rail content, which the shell's `#inspector-rail` shows in place of the item inspector while the screen is active (`[data-wb-screen-aside]`, CSS only in `wb-tokens.css`; an item opened from the screen, e.g. a developer tool's Details, takes the rail back). `admin-shell.ts` is untouched: the panel, its `#wb-screen-title-org` (id, text, focus) and `[data-wb-screen-body]` stay the shell's, only their classes change.
- Deployment setup and ECC hook controls rebuilt in admin-org.html's language: the form card, mono uppercase labels, the `from git`-style count pill, CLI chips as the prototype's email chips (pressed = primary border), a styled `#posture` select with its chevron, the managed-MCP checkbox drawn as the prototype's toggle switch, disclosure chevrons, the ECC profile radios as a segmented control, hook rows as bordered rows with filled small buttons. Rail: "In this policy" (posture, allowed CLI, managed MCP, ECC hook profile — read from the current policy), "In the organization policy file" (the policy's posture/governance excerpt, keys and strings coloured as the prototype, every piece a text node), "On a user's machine" and the info note. Every id and `data-*` hook kept; the density assertions (setup < 180 px, ≤ 24 px under the title, ECC card < 300 px down) still hold.
- Not built (no product data, NEW-SHELL-PLAN.md §2 Omit): first-time steps rail, lede, organization name / short id / policy repo / people fields, Vibe/Enterprise signing cards, "Save and pick a template". The shell's nav rail and kind ledger stay (every admin screen).
- Screenshots: `screenshots/fidelity-org-{proto,product}-{1440-dark,1440-light,375}.png`. The prototype is not responsive at 375. Visual baselines `new-shell-org-{light,dark}` regenerated.
- Functional walk (Playwright, `aih-policy-workbench.html`, 1440): Enterprise refused without a CLI (message, posture back to Vibe); CLI chip on/off (pressed state, "1 selected", `governance.supportedClis`, rail updates); Enterprise accepted with a CLI; managed MCP switch on/off (both messages); adoption recipe opens, Escape closes it and returns focus to the toggle; Evidence & versions opens, Escape closes it and focuses its summary; info tooltip on hover, Escape closes; developer tools expand, MarkItDown exclude/include written to `developerTools.excluded`; ECC hooks expand, profile `minimal` recorded, `pre:bash:block-no-verify` disabled and re-enabled; developer tool Details opens the item inspector over the rail (developer-tools.spec). Zero page errors, zero network requests; 375 no horizontal scroll.

## Fidelity port — additions screen (admin-acme.html)

- Same frame (`shell/screen-frame.ts`): kicker, 22 px title and lede. The cards take admin-acme.html's step card (`bg-[#141822]` = `bg-wb-card`, hairline border, `p-3.5`), its badge + bold 13.5 px title (numbered 1 for organization artifacts and 2 for the protected policy file, i.e. "say what it is / scan it" and "read the result and decide"; icon badges for Bring Your Own, curation, your sources, ECC MCP approval). Fields: mono uppercase labels over mono `surface-container-lowest` inputs and selects with a primary focus border; filled small buttons (`surface-container`), primary buttons in the prototype's blue, dashed pop rows, command wells, disclosure triangles. The constant protected-form / custom-MCP markup and the artifact intake runtime's DOM keep their bytes and legacy class names; `wb-tokens.css`'s `[data-wb-acme]` / `#artifact-intake-review` block (S7) is rewritten to the same language. The protected card's grphead gets the step-2 badge in the clone (DOM only).
- Rail: "In this policy" (custom MCP, remote MCP, framework curation, ECC MCP approvals — counted from the policy), "States an addition can be in" (the product's own row states: Blocked / Requested intent / Disabled / External guidance) and the pin note.
- Not built (no product data): the left "Acme · your own source" list, the kind segmented control (the intake runtime has its own kind select), the scan result drop zone and Allow / Hold back toggle, "What users will see" source counts, "See how findings read".
- Screenshots: `screenshots/fidelity-acme-{proto,product}-{1440-dark,1440-light,375}.png`. Baselines `new-shell-acme-{light,dark}` regenerated.
- Functional walk (Playwright, 1440; `aih-policy-workbench.html`, and `golden-downloads.html` for the byte gates): Bring Your Own hook note opens, Escape closes it and returns focus to its opener; ECC MCP panel opens with focus on its select, Escape closes; curation Add with empty fields → error announced, `#curation-id` `aria-invalid`; valid add, Edit (button becomes "Save framework curation"), Cancel, Remove; custom MCP with a bad owner email is blocked (native `type=email`), then added ("Pending custom MCP added…", rail count 1); remote MCP with an `http://…/path` origin → "Correct the highlighted remote-endpoint fields." and `aria-invalid`, then an HTTPS origin records it; artifact intake Import (3 rows) and Download = golden `aih-artifact-intake.json` byte for byte; protected form empty submit blocked by the native `required` fields, then built, Download = golden `aih-policy-bundle.json` byte for byte; zero network requests; 375 no horizontal scroll. `new-shell.spec` repeats both byte gates under the strict CSP with zero violations.
- Found, not fixed (constant markup in `studio-custom-mcp.ts`, pre-existing): Chromium logs "Pattern attribute value [a-z][a-z0-9-]{0,63} is not a valid regular expression" for `#custom-id` / `#remote-custom-id` (the `v` flag needs `\-`), so that `pattern` is ignored in the browser; the JS validators still enforce the id. Also on the default fixture, after custom candidates are added the protected form answers "Correct the highlighted protected policy fields." for the same inputs that build on `golden-downloads.html`; left as policy semantics.
