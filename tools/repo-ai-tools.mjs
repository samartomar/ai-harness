...
function writeCodexProjection() {
  ...
  if (begin >= 0 || end >= 0) {
    ...
    writeFileSync(codexConfigPath, updated.endsWith("\n") ? updated : `${updated}\n`, "utf8");
    return;
  }
  ...
  writeFileSync(codexConfigPath, `${existing}${separator}${expected}`, "utf8");
}
...
function initializeCodebaseMemory() {
  ...
  if (existsSync(codebaseMemoryMarker)) {
    const marker = parseJson(readFileSync(codebaseMemoryMarker, "utf8"), "codebase-memory marker");
