...
function writeCodexProjection() {
  ...
  let existing = readOptionalUtf8(codexConfigPath);
  if (existing === undefined) {
    try {
      writeNewFileExclusively(codexConfigPath, expected);
      return;
    } catch (error) {
      if (!hasErrorCode(error, "EEXIST")) throw error;
      existing = readOptionalUtf8(codexConfigPath);
      if (existing === undefined) throw new Error("Codex projection changed during exclusive create");
    }
  }
  ...
  if (begin >= 0 || end >= 0) {
    ...
    writeFileAtomically(codexConfigPath, updated.endsWith("\n") ? updated : `${updated}\n`);
    return;
  }
  if (existing.startsWith("# Generated by: npm run repo:init")) {
    writeFileAtomically(codexConfigPath, expected);
    return;
  }
  for (const server of projectMcpServers) {
  ...
  writeFileAtomically(codexConfigPath, `${existing}${separator}${expected}`);
}
