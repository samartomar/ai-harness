...
const receipt = {
  ...
  sampling: {
    metric: "summed process-tree resident bytes",
    intervalMs: 250,
    includesOsQueryOverhead: true,
  },
  stages: [],
};
...
async function recordStage(name, args, budgetMs) {
  const metrics = await measureProcess(process.execPath, args);
  let preparation;
  if (name === "preparation") {
    preparation = JSON.parse(await readFile(resolve(directory, "preparation.json"), "utf8"));
  }
  receipt.stages.push({
    name,
    ...
    ...(budgetMs ? { budgetMs } : {}),
    ...(preparation ? {
      children: preparation.children,
      ...(preparation.interruption ? { interruption: preparation.interruption } : {}),
    } : {}),
  });
  ...
  if (metrics.code !== 0 || metrics.signal) {
    throw new Error(name + " failed with " + (metrics.signal ? "signal " + metrics.signal : "exit " + metrics.code));
  }
